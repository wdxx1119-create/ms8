...
SEMANTIC_SHORT_ALLOWLIST = {"改成", "启用", "禁用", "方案", "缓存", "sqlite", "mysql", "postgres", "redis", "回滚", "发布", "阈值"}
...
def evaluate_block(text: str) -> Dict[str, str | bool]:
    ...
    if not stripped:
        return {"blocked": True, "reason": "empty_or_whitespace", "severity": "high", "suggested_route": "rejected"}
    if stripped in SYSTEM_PLACEHOLDERS:
        return {"blocked": True, "reason": "system_placeholder", "severity": "high", "suggested_route": "rejected"}
    if re.fullmatch(r"\d+", stripped):
        return {"blocked": True, "reason": "numeric_only", "severity": "high", "suggested_route": "rejected"}
    if re.fullmatch(r"[a-z]+\d+|\d+[a-z]+", normalized, flags=re.IGNORECASE) and len(normalized) <= 10:
        return {"blocked": True, "reason": "alnum_noise", "severity": "medium", "suggested_route": "rejected"}
    if re.fullmatch(r"[^\w\s]+", stripped):
        return {"blocked": True, "reason": "punctuation_only", "severity": "high", "suggested_route": "rejected"}
    if _is_emoji_only(stripped):
        return {"blocked": True, "reason": "emoji_only", "severity": "high", "suggested_route": "rejected"}
    ...
    if emoji_ratio >= 0.5 and len(non_emoji_compact) <= 1:
        return {"blocked": True, "reason": "emoji_mixed_low_value", "severity": "medium", "suggested_route": "rejected"}
    if _is_repeated_noise(stripped):
        return {"blocked": True, "reason": "repeated_chars_noise", "severity": "medium", "suggested_route": "rejected"}
    for pattern in BLOCK_PATTERNS:
        if re.search(pattern, stripped, flags=re.IGNORECASE) or re.search(pattern, normalized, flags=re.IGNORECASE):
            return {"blocked": True, "reason": "template_ack", "severity": "medium", "suggested_route": "rejected"}
    ...
    if any(compact == cmd or compact.startswith(cmd) or compact.endswith(cmd) for cmd in LOW_VALUE_COMMANDS):
        if not any(tok in compact for tok in SEMANTIC_SHORT_ALLOWLIST):
            return {"blocked": True, "reason": "short_low_value_command", "severity": "low", "suggested_route": "short_term_only"}
