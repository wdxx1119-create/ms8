...
class MemoryPipeline:
    ...
    def process(self, text: str, source: str = "interaction") -> PipelineResult:
        ...
        trace.add(
            "extraction",
            {"signals": signals, "entities": entities, "action": action, "object": obj, "status": status},
        )
        ...
        if admission.route == "pending_review":
            ...
            needs_review = True
            review_reason = review_reason if review_reason and review_reason != "accepted" else "admission_pending_review"
        elif admission.route == "redacted_accept":
            if save_status == "rejected":
                save_status = "pending_review"
                needs_review = True
            review_reason = review_reason if review_reason and review_reason != "accepted" else "admission_redacted_accept"
        ...
        if needs_review:
            ...
            risk_level = "high" if (conflict_flag or "privacy" in str(review_reason).lower()) else ("medium" if confidence < 0.7 else "low")
            effective_review_reason = str(review_reason or "").strip() or "unspecified_review"
            self.review_service.enqueue(
                ReviewItem(
                    memory_id=str(saved.get("id") or saved.get("meta", {}).get("id", "")),
