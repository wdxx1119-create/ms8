...
def _pipeline_log_candidates(memory_dir: Path) -> List[Path]:
    ...
    if logs_dir.exists():
        for p in sorted(logs_dir.glob("auto_memory_pipeline*.log"), key=lambda x: x.stat().st_mtime, reverse=True):
            if p not in candidates:
                candidates.append(p)
    return candidates
...
def _check_l1_launchd_mcp(_core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    label = "com.openclaw.memory.mcp"
    if _launchctl_running(label):
        return _ok("launchd mcp service running", {"label": label})
    return _fail("launchd mcp service not running", {"label": label})


def _check_l1_launchd_maintenance(_core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    label = "com.openclaw.memory.maintenance"
    if _launchctl_running(label):
        return _ok("launchd maintenance service running", {"label": label})
    return _warn("launchd maintenance service not running", {"label": label})
...
def _check_l1_disk_space(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    mp_cfg = mem_settings.get("maintenance_policy", {}).get("self_check", {}) if isinstance(mem_settings.get("maintenance_policy", {}), dict) else {}
...
def _check_l1_health_card_diff(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    from .reporter import build_health_card, _diff_health_card  # local import to avoid heavy import cycles
    ...
    try:
        ...
        return _warn("health card target unreadable", {"path": str(card), "compare_target": compare_target, "error": str(exc)})
...
def _check_l1_self_check_framework(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    stale_hours = float(core.config["settings"]["memory"].get("monitoring", {}).get("alerts", {}).get("self_check_stale_hours", 2) or 2)
...
def _connect_root(core: Any) -> Path:
    ...
    root = str(cfg.get("root", os.environ.get("OPENCLAW_MEMORY_AUTO_ROOT", "~/openclaw-memory-auto")))
...
def _check_c1_mcp_server_smoke(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not health:
        return _warn("connect health report missing", {"path": str(root / "runtime" / "health.json"), "label": label})
...
def _check_c2_mcp_tool_contract(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return _ok("mcp tool contract healthy", {"methods": methods, "status_ok": bool(status.get("ok", False))})
    except Exception as exc:
...
def _check_c4_adapter_registry_integrity(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for name, item in payload.items():
        ...
        if "capabilities" not in item:
            bad.append(name)
    if bad:
        return _warn("adapter registry has malformed entries", {"malformed": bad, "active": active, "total": len(payload)})
...
def _check_c5_interface_single_entry(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "from mcp_server.memory_service_interface import MemoryServiceInterface" not in atext:
        violations.append("adapter_missing_service_interface")
    if "from mcp_server.memory_service_interface import MemoryServiceInterface" not in mtext:
...
def _check_c6_client_config_presence(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, path in targets.items():
        ...
        details[key] = {"path": str(path), "present": present, "snippet": str(snippets[key]), "snippet_present": snippets[key].exists()}
    if not has_any:
...
def _check_c7_source_tagging_e2e(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for ln in lines:
        ...
        if "save_memory" not in low and "source" not in low:
            continue
        total += 1
        ok = ("source=mcp:" in low) or ("source=adapter:" in low) or ("\"source\": \"mcp:" in low) or ("\"source\": \"adapter:" in low)
        if ok:
            tagged += 1
        if len(samples) < 5:
...
def _check_c8_connect_report_health(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    details = {"overall_ok": overall_ok, "steps": len(steps), "failed_steps": len(failed_steps)}
    if overall_ok:
...
def _check_l2_admission_distribution(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not candidates:
        return _ok("no pipeline log available for probe, skipping", {"path": str(memory_dir / "auto_memory_pipeline.log")})
...
def _check_l2_write_then_search(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    probe_text = f"self check write then search probe {probe_id} keep pipeline stable"
    details: Dict[str, Any] = {"probe_id": probe_id, "source": probe_source, "residue_clean": False}

    try:
        result = pipeline.process(probe_text, source=probe_source)
        details["pipeline_status"] = getattr(result, "status", "")
        details["pipeline_records"] = len(getattr(result, "records", []) or [])
...
def _check_l2_repair_audit_health(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for ln in tail:
        ...
        try:
            ...
            bad += 1
    if bad > 0:
        return _warn("repair audit has malformed rows", {"path": str(p), "bad_rows": bad, "tail_rows": len(tail)})
...
def _check_l3_repair_effectiveness(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    details = {"total_7d": total, "success_rate_7d": success_rate, "rollback_rate_7d": rollback_rate}
    if total <= 0:
...
def _check_l1_self_check_integrity(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not baseline.exists():
        ...
        return _warn("self-check integrity baseline initialized", {"baseline": str(baseline), "hashes": current})
    try:
        payload = json.loads(baseline.read_text(encoding="utf-8"))
    except Exception as exc:
        return _fail("self-check integrity baseline unreadable", {"baseline": str(baseline), "error": str(exc)})
...
def _check_l3_checkpoint_verify(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    out = core.shadow_verify() if hasattr(core, "shadow_verify") else {"ok": False, "reason": "shadow_verify_unavailable"}
...
def _check_l3_shadow_permissions(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    try:
        from memory.security.shadow.shadow_permissions import ensure_shadow_permissions
        ...
        if violations and corrected:
            return _warn("shadow permissions corrected", {"violations": len(violations), "corrected": len(corrected)})
...
def _check_l3_backup_freshness(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    last = str(
        payload.get("last_backup_at", "")
        or payload.get("last_backup", "")
        or payload.get("backup_last_run", "")
    ).strip()
    if not last:
...
def _check_l3_closed_loop_evidence(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    details = {"samples": total, "complete_chain": complete, "complete_ratio": ratio, "missing_examples": missing_examples}
    if ratio < 0.70:
...
def _check_l4_capture_trend(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for e in entries:
        ...
        # not extraction outcomes, and should not dilute capture quality.
        if st not in {"success", "dropped", "rejected", "duplicate"}:
            continue
        bucket = day_stats.setdefault(day, {"total": 0, "success": 0})
        bucket["total"] += 1
        if st == "success":
            bucket["success"] += 1
    ...
    ratios = [day_stats[d]["success"] / max(1, day_stats[d]["total"]) for d in days]
    if len(ratios) < 3:
        return _ok("capture trend insufficient samples", {"days": len(ratios), "avg": round(sum(ratios)/len(ratios), 4)})
    ...
    details = {"days": len(ratios), "avg": avg, "delta": delta, "head": round(head,4), "tail": round(tail,4)}
    if avg < 0.60:
...
def _check_l4_injection_effectiveness(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for line in rows:
        ...
        if int(row.get("injected_count", row.get("count", 0)) or 0) > 0:
            used += 1
    if total == 0:
        return _warn("no injection samples", {})
    ratio = round(used / max(1, total), 4)
    details = {"samples": total, "used_ratio": ratio}
    if ratio < 0.40:
...
def _check_m1_short_term_persistence(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if lines == 0:
        return _warn("working memory persistence empty", {"path": str(wm_file), "restore_api": has_restore_api})
...
def _check_m4_dedupe_false_positive_probe(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    candidates = _pipeline_log_candidates(memory_dir)
    if not candidates:
        return _ok("no pipeline log available for dedupe probe, skipping", {"path": str(memory_dir / "auto_memory_pipeline.log")})
    path = candidates[0]
    total = 0
    dup_drop = 0
    hi_conf_dup_drop = 0
    ...
    for ln in rows:
        ...
        if "duplicate" in reason_text:
            dup_drop += 1
            conf = float(row.get("confidence", row.get("record", {}).get("confidence", 0.0)) or 0.0) if isinstance(row, dict) else 0.0
            if conf >= 0.75:
                hi_conf_dup_drop += 1
    if total == 0:
    ...
    details = {"samples": total, "duplicate_drop_ratio": ratio, "high_conf_duplicate_ratio": hi_ratio}
...
def _check_m6_cjk_recall_probe(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    details = {"query_full": q_full, "query_part": q_part, "full_hits": full_hits, "part_hits": part_hits}
...
def _check_m8_pipeline_latency_budget(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not samples:
        if row_count > 0:
            return _ok("pipeline active but latency instrumentation unavailable", {"rows": row_count, "samples": 0})
...
def _check_s1_keychain_effective(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    details = {"use_keychain": use_keychain, "manifest_hmac_file_exists": exists, "path": str(key_file)}
...
def _check_s3_shadow_immutable_flags(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    imm_enabled = bool(core.config["settings"]["memory"].get("security", {}).get("shadow", {}).get("immutable_enabled", False))
    shadow_dir = Path(core.config["memory_dir"]) / "security" / "shadow_data"
...
def _check_s4_shadow_backup_dual_site(core: Any, _ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    details = {"primary_exists": primary.exists(), "backup_exists": backup.exists(), "primary": str(primary), "backup": str(backup)}
...
def build_check_specs(level: str = "L1") -> List[CheckSpec]:
    l = str(level or "L1").upper()
    specs: List[CheckSpec] = [
        CheckSpec("l1_launchd_mcp", "L1", 5, "Run launchctl kickstart gui/$(id -u)/com.openclaw.memory.mcp", _check_l1_launchd_mcp, domain="connect"),
        CheckSpec("l1_launchd_maintenance", "L1", 5, "Run ocma install-agent to register maintenance launch agent", _check_l1_launchd_maintenance, domain="memory"),
        CheckSpec("l1_core_files", "L1", 5, "Restore missing core files from backups", _check_l1_core_files, domain="memory"),
        CheckSpec("l1_shadow_files", "L1", 5, "Run shadow health and repair missing survival files", _check_l1_shadow_files, domain="security"),
        CheckSpec("l1_canary_io", "L1", 5, "Check disk permission and free space", _check_l1_canary_io, domain="memory"),
        CheckSpec("l1_disk_space", "L1", 5, "Cleanup old backups and archives", _check_l1_disk_space, domain="memory"),
        CheckSpec("l1_health_card_diff", "L1", 5, "Run full self-check and refresh health card", _check_l1_health_card_diff, domain="memory"),
        CheckSpec("l1_shadow_sealed", "L1", 5, "Inspect shadow status and recover if persistent sealed", _check_l1_shadow_sealed, domain="security"),
        CheckSpec("l1_self_check_framework", "L1", 5, "Verify self-check report+heartbeat freshness", _check_l1_self_check_framework, domain="memory"),
        CheckSpec("l1_check_coverage", "L1", 5, "Ensure minimum self-check coverage remains intact", _check_l1_check_coverage, domain="memory"),
        CheckSpec("l1_self_check_integrity", "L1", 5, "Verify self-check module integrity hashes", _check_l1_self_check_integrity, domain="security"),
        CheckSpec("l2_pipeline_stages", "L2", 60, "Inspect pipeline dependencies and runtime loading", _check_l2_pipeline_stages, domain="memory"),
        CheckSpec("l2_admission_distribution", "L2", 60, "Review admission policy thresholds", _check_l2_admission_distribution, domain="memory"),
        CheckSpec("l2_write_then_search", "L2", 60, "Run write->search->cleanup closed-loop probe for ingestion/index/retrieval", _check_l2_write_then_search, domain="memory"),
        CheckSpec("l2_index_consistency", "L2", 60, "Run index rebuild to close consistency gap", _check_l2_index_consistency, domain="memory"),
        CheckSpec("l2_sqlite_integrity", "L2", 60, "Restore databases from latest healthy backup", _check_l2_sqlite_integrity, domain="memory"),
        CheckSpec("l2_kg_orphan_check", "L2", 60, "Run knowledge graph relation cleanup", _check_l2_kg_orphan_check, domain="memory"),
        CheckSpec("l2_jsonl_parse", "L2", 60, "Run JSONL self-heal or truncate bad tails", _check_l2_jsonl_parse, domain="memory"),
        CheckSpec("l2_slo_check", "L2", 60, "Inspect monitoring SLO targets and recent breaches", _check_l2_slo_check, domain="memory"),
        CheckSpec("l2_repair_audit_health", "L2", 60, "Verify self-repair audit health and writeability", _check_l2_repair_audit_health, domain="memory"),
        CheckSpec("l2_repair_lock_health", "L2", 60, "Verify self-repair in-progress lock freshness", _check_l2_repair_lock_health, domain="memory"),
        CheckSpec("m1_short_term_persistence", "L2", 60, "Verify short-term memory persistence and restore readiness", _check_m1_short_term_persistence, domain="memory"),
        CheckSpec("m2_rejected_not_indexed", "L2", 60, "Ensure rejected records are excluded from main index", _check_m2_rejected_not_indexed, domain="memory"),
        CheckSpec("m3_review_queue_sla", "L2", 60, "Check review queue backlog age and pending volume", _check_m3_review_queue_sla, domain="memory"),
        CheckSpec("m4_dedupe_false_positive_probe", "L2", 60, "Probe possible dedupe false-positive risk", _check_m4_dedupe_false_positive_probe, domain="memory"),
        CheckSpec("m5_semantic_cache_health", "L2", 60, "Check semantic cache dense vector completeness", _check_m5_semantic_cache_health, domain="memory"),
        CheckSpec("m6_cjk_recall_probe", "L2", 60, "Probe CJK recall quality on concatenated queries", _check_m6_cjk_recall_probe, domain="memory"),
        CheckSpec("m7_kg_access_feedback", "L2", 60, "Check KG access_count feedback effectiveness", _check_m7_kg_access_feedback, domain="memory"),
        CheckSpec("m8_pipeline_latency_budget", "L2", 60, "Check pipeline p95 latency budget", _check_m8_pipeline_latency_budget, domain="memory"),
        CheckSpec("c1_mcp_server_smoke", "L2", 60, "Validate MCP server smoke health report", _check_c1_mcp_server_smoke, domain="connect"),
        CheckSpec("c2_mcp_tool_contract", "L2", 60, "Validate service interface tool contract", _check_c2_mcp_tool_contract, domain="connect"),
        CheckSpec("c3_mcp_resource_contract", "L2", 60, "Validate service interface resources contract", _check_c3_mcp_resource_contract, domain="connect"),
        CheckSpec("c4_adapter_registry_integrity", "L2", 60, "Validate adapters registry schema and active entries", _check_c4_adapter_registry_integrity, domain="connect"),
        CheckSpec("c5_interface_single_entry", "L2", 60, "Enforce service interface single-entry path", _check_c5_interface_single_entry, domain="connect"),
        CheckSpec("c6_client_config_presence", "L2", 60, "Verify client MCP config presence", _check_c6_client_config_presence, domain="connect"),
        CheckSpec("c7_source_tagging_e2e", "L2", 60, "Verify source tagging in connect audit stream", _check_c7_source_tagging_e2e, domain="connect"),
        CheckSpec("c8_connect_report_health", "L2", 60, "Verify connect report overall health", _check_c8_connect_report_health, domain="connect"),
        CheckSpec("l3_manifest_signature", "L3", 120, "Run shadow seal/reset checkpoint if signature invalid", _check_l3_manifest_signature, domain="security"),
        CheckSpec("l3_checkpoint_verify", "L3", 120, "Run ocma shadow reset-checkpoint", _check_l3_checkpoint_verify, domain="security"),
        CheckSpec("l3_shadow_permissions", "L3", 120, "Repair shadow_data permissions to 700/600", _check_l3_shadow_permissions, domain="security"),
        CheckSpec("l3_shadow_health", "L3", 120, "Run shadow health and inspect issues list", _check_l3_shadow_health, domain="security"),
        CheckSpec("l3_seal_history", "L3", 120, "Inspect 24h seal frequency and root causes", _check_l3_seal_history, domain="security"),
        CheckSpec("l3_spool_backlog", "L3", 120, "Replay shadow spool backlog and verify drain", _check_l3_spool_backlog, domain="security"),
        CheckSpec("l3_closed_loop_evidence", "L3", 120, "Validate trigger/action/result/evidence chain", _check_l3_closed_loop_evidence, domain="memory"),
        CheckSpec("l3_repair_effectiveness", "L3", 120, "Evaluate self-repair 7-day effectiveness", _check_l3_repair_effectiveness, domain="memory"),
        CheckSpec("l3_backup_freshness", "L3", 120, "Run shadow backup-sync and maintenance backup", _check_l3_backup_freshness, domain="security"),
        CheckSpec("l3_sensitive_scan", "L3", 120, "Move secrets to keychain and redact persisted plaintext", _check_l3_sensitive_scan, domain="security"),
        CheckSpec("s1_keychain_effective", "L3", 120, "Verify keychain-backed shadow key and file-key cleanup", _check_s1_keychain_effective, domain="security"),
        CheckSpec("s2_shadow_ops_audit_writable", "L3", 120, "Verify shadow ops_audit appendability", _check_s2_shadow_ops_audit_writable, domain="security"),
        CheckSpec("s3_shadow_immutable_flags", "L3", 120, "Verify uchg immutable protection on key shadow files", _check_s3_shadow_immutable_flags, domain="security"),
        CheckSpec("s4_shadow_backup_dual_site", "L3", 120, "Verify shadow dual-site backup availability", _check_s4_shadow_backup_dual_site, domain="security"),
        CheckSpec("s5_replay_dryrun_weekly", "L3", 120, "Verify weekly replay/drill cadence evidence", _check_s5_replay_dryrun_weekly, domain="security"),
        CheckSpec("s6_manifest_checkpoint_pair", "L3", 120, "Verify manifest signature + checkpoint gate", _check_s6_manifest_checkpoint_pair, domain="security"),
        CheckSpec("s7_locking_lease_health", "L3", 120, "Verify shadow locking lease health", _check_s7_locking_lease_health, domain="security"),
        CheckSpec("s8_minimal_survival_trigger", "L3", 120, "Verify minimal_survival trigger path", _check_s8_minimal_survival_trigger, domain="security"),
        CheckSpec("l4_capture_trend", "L4", 300, "Check auto_memory extraction quality trend and adjust rules", _check_l4_capture_trend, domain="memory"),
        CheckSpec("l4_injection_effectiveness", "L4", 300, "Tune injection budget and ranking weights", _check_l4_injection_effectiveness, domain="memory"),
        CheckSpec("l4_threshold_suggestions", "L4", 300, "Review and approve/reject pending threshold suggestions", _check_l4_threshold_suggestions, domain="memory"),
        CheckSpec("l4_capacity_projection", "L4", 300, "Plan cleanup/archival or storage expansion", _check_l4_capacity_projection, domain="memory"),
    ]
    if l == "L1":
