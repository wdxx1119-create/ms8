...
from .reporter import load_latest as reporter_load_latest, persist_report
...
def run_self_check(core: Any, level: str = "L1") -> Dict[str, Any]:
    ...
    heartbeat_path = Path(config["settings"]["memory"].get("self_check", {}).get("heartbeat_path", "/tmp/ocma_self_check_heartbeat"))
    ...
    try:
        ...
        ctx: Dict[str, Any] = {"workspace_dir": str(config["workspace_dir"]), "memory_dir": str(memory_dir)}
        for spec in checks:
        ...
        report = {
            ...
            "status": "ok" if int(summary.get("exit_code", 1)) == 0 else ("warning" if int(summary.get("exit_code", 1)) == 1 else "failed"),
