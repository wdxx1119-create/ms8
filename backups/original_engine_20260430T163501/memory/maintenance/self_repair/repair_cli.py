...
def main() -> int:
    parser = argparse.ArgumentParser(description="openclaw-memory self-repair")
    ...
    if cmd == "status":
        ...
        return 0
    if cmd == "history":
        _print({"status": "ok", "rows": list_repair_history(core.config["memory_dir"], limit=int(args.limit))})
        return 0
    if cmd == "rollback":
