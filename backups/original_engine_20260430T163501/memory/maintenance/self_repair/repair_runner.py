    ...
    *,
    ...
    try:
        if action in {"restart_launchd_mcp", "restart_launchd_maintenance"}:
            ...
            out["launchd"] = {"label": label, "returncode": int(cp.returncode), "stderr": str(cp.stderr or "")[-400:]}
            if cp.returncode != 0:
        ...
        if verify_result and str(verify_result.get("status", "")).lower() in {"error", "fail", "failed"}:
...
def _load_dynamic_chain_rules(cfg_sc: Dict[str, Any]) -> List[Dict[str, Any]]:
    chain_cfg = cfg_sc.get("dynamic_repair_chain", {}) if isinstance(cfg_sc.get("dynamic_repair_chain", {}), dict) else {}
...
def _notify_repair_summary(report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    r3_success = sum(
        1
        for x in executed
        if str(x.get("result", "")) == "success" and str(x.get("risk", "")).upper() == "R3"
    )
    ...
    try:
        ...
        return {"status": "error", "error": str(exc), "failed": failed, "needs_manual": manual, "r3_success": r3_success}
...
def run_repair_plan(core: Any, plan_payload: Dict[str, Any], *, mode: str = "dry-run") -> Dict[str, Any]:
    ...
    if not got_lock:
        return {
            ...
            "summary": {"planned": 0, "executed": 0, "success": 0, "failed": 1, "rolled_back": 0, "needs_manual": 1},
            "executed": [],
        }
            ...
            for row in rows:
                ...
                try:
                    ...
                    if not ok:
                        ...
                        rolled_back = True
                        rb_verify = _rollback_verify(memory_dir, before.get("probe", {}), action)
