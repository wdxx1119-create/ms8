...
class PatternRecognition:
    ...
    async def analyze_conversations(self, 
                                   ...
                                   analyze_patterns: bool = True) -> Dict:
        """
        分析对话，识别模式
    ...
    async def _llm_detect_patterns(self, conversations: List[Dict]) -> List[Pattern]:
        ...
        if self.llm is None:
            ...
            user_messages = [c.get("content", "") for c in conversations if c.get("role") == "user" and c.get("content")]
...
"""
    ...
    def _parse_emotion_response(self, response: str, context: str) -> Optional[EmotionAnalysis]:
        """解析 LLM 情感分析响应"""
        try:
            import re
            json_match = re.search(r'\{[\s\S]*\}', response)
    ...
    def _calculate_statistics(self, conversations: List[Dict]) -> Dict:
        """计算统计信息"""
        stats = {
    ...
    def detect_cross_conversation_links(self, 
    ...
    def _find_related_conversations(self, 
                                    content: str, 
