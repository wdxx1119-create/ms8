...
class KnowledgeFeedbackRecorder:
    ...
    def record_admission(self, knowledge_id: str, tier: str, trust: str, accepted_or_rejected: str, event: str, extra: Dict[str, Any] | None = None) -> None:
        self._append(
            {
                "event": event,
    ...
    def _effective_level(self, row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        tier = str(row.get("tier", "observation"))
        ...
        if score >= hard_min:
            return {"effective_trust": "hard_trust", "effective_tier": "core", "effective_score": score}
        if score >= soft_min:
            return {"effective_trust": "soft_trust", "effective_tier": "graph", "effective_score": score}
        if score >= hypo_min:
            return {"effective_trust": "hypothesis", "effective_tier": "observation", "effective_score": score}
        return {"effective_trust": "isolated", "effective_tier": "rejected", "effective_score": score}
    ...
    def rebuild_balanced_feedback(self, window: int | None = None) -> Dict[str, Any]:
        ...
        if total == 0:
            ...
            return {"status": "empty", "total": 0, "rebalanced": 0, "output_file": str(self.rebalance_file)}
        ...
        if shape_enabled and merged_rows:
            ...
            ranked = sorted(merged_rows, key=lambda x: self._to_float(x.get("effective_score", 0.0), 0.0), reverse=True)
