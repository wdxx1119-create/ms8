...
class SelfImprovementEngine:
    ...
    def __init__(self, memory_core, config_dir: str = None):
        """
    ...
    def _determine_target_block(self, instruction: str, content: Optional[str] = None) -> str:
        """Route remember instructions to the most relevant memory block."""
        combined = f"{instruction} {content or ''}".lower()
        human_keywords = [
            'user', 'name', 'prefer', 'like', 'want', 'works on', 'uses',
            'prefers', 'favorite', 'habit', 'timezone', 'project',
        ]
        persona_keywords = [
            'i am', "i'm", 'persona', 'style', 'tone', 'role',
            'assistant should', 'always answer', 'never answer',
        ]
    ...
    def _generate_edit_reason(self, 
                              instruction: str, 
                              context: List[Dict],
                              block_type: str = 'archival') -> str:
        """
    ...
    def _test_relevance(self, improvement: ImprovementRecord) -> float:
        ...
        meaningful_keywords = [
            'prefer', 'like', 'want', 'need', 'use', 'avoid',
            'always', 'never', 'sometimes', 'usually', 'project',
            'timezone', 'works', 'style', 'tone',
        ]
        ...
        if improvement.validation_score:
            ...
            if total > 0:
                self.metrics['average_score'] = (
                    (self.metrics['average_score'] * (total - 1) + improvement.validation_score) / total
                )
