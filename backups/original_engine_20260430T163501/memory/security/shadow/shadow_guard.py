...
class ShadowSystem:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        default_immutable = not (
            shadow_dir_str.startswith("/tmp")
            or shadow_dir_str.startswith("/var/folders/")
        )
        ...
        try:
            from memory.security.encryption.crypto_manager import get_crypto_manager
        ...
        def _admission_check(text: str, metadata: Dict[str, Any]) -> Dict[str, Any]:
            try:
                ...
                return evaluate_candidate(
                    ...
                    candidate_type="shadow_recovery",
        ...
        ensure_shadow_permissions(self.shadow_dir, backup_dir=self.backup_dir, audit_cb=lambda e, m: self.record_mode("protect", source="shadow:permissions", ok=True, metadata={"event": e, **m}))
    ...
    def bind_recovery_target(
        ...
        hash_exists_func: Optional[Callable[[str], bool]] = None,
    ) -> None:
        self.recovery.bind_target(target, write_func, hash_exists_func)
        self._bound_targets[str(target)] = True

    def _req(self, caller_id: str, request_reason: str, request_token: Optional[str] = None) -> GateRequest:
        token = str(request_token or self._default_tokens.get(caller_id, ""))
        return GateRequest(caller_id=str(caller_id), request_reason=str(request_reason or "manual"), request_token=token)
    ...
    def _check_backpressure(self) -> Dict[str, Any]:
        ...
        if self.minimal_survival_enabled and ratio >= self.minimal_survival_enter_pct and mode != "minimal_survival":
            ...
            return {"entered": True, "status": st, "ratio": ratio, "usage": usage, "stage": eval_out.get("stage", "critical")}
        if mode == "minimal_survival" and ratio <= self.minimal_survival_exit_pct:
            ...
            return {"exited": True, "status": st, "ratio": ratio, "usage": usage, "stage": eval_out.get("stage", "ok")}
    ...
    def trigger_seal(
        ...
        *,
        ...
        def _commit(_lease_id: str) -> Dict[str, Any]:
            ...
            return {"status": "success", "post_state": str(out.get("mode", self._seal.mode())), "result": out}
    ...
    def clear_seal(
        ...
        *,
        ...
        if not self._stack_guard_ok():
            return {"status": "rejected", "reason": "stack_guard_blocked"}
        if not bool(confirm):
            self.record_mode("protect", source="shadow:control_gate", ok=False, error="clear_seal_rejected", metadata={"reason": reason})
        ...
        if not want_reason or ((not want_session) and (not legacy_no_session)):
            self.record_mode(
                ...
                metadata={"reason": "missing_seal_identity", "seal_reason": cur_reason, "seal_session_id": cur_session},
            )
            return {
                "status": "rejected",
    ...
    def handle_write_error(self, reason: str, source: str = "shadow:auto") -> Dict[str, Any]:
        ...
        if bool(out.get("promoted_to_hard", False)):
            self.record_mode(
                ...
                source=source,
                ok=True,
                metadata={"reason": reason, "seal_level": "hard", "trigger": "soft_to_hard_promotion"},
            )
        return out
    ...
    def spool_write(self, content: str, source: str = "shadow:sealed") -> Dict[str, Any]:
        ...
        if str(self._seal.mode()) == "minimal_survival":
            ...
            self.record_mode(
                ...
                source="shadow:capacity_guard",
                ok=True,
                metadata={"event": "minimal_survival_spool", "spool_id": item.get("spool_id"), "capacity": bp},
            )
            return item
        ...
        *,
        ...
        def _commit(lease_id: str) -> Dict[str, Any]:
            ...
            if not bool(gate.get("ok", False)):
                ...
                return {"status": "blocked", "post_state": self._seal.mode(), "error": str(gate.get("reason", "checkpoint_mismatch")), "failed": 0, "replayed": 0, "skipped": 0, "remaining": 0}
            out = self.recovery.replay_spool(target=target)
            if not self.locking.validate_lease(lease_id):
                return {"status": "partial", "error": "lease_expired_midflight", "post_state": self._seal.mode(), **out}
            self.record_mode(
                "recover",
            ...
            return {"status": str(out.get("status", "success")), "post_state": self._seal.mode(), **out}
        ...
        *,
        ...
        def _commit(lease_id: str) -> Dict[str, Any]:
            ...
            if not bool(gate.get("ok", False)):
                ...
                return {"status": "blocked", "post_state": self._seal.mode(), "error": str(gate.get("reason", "checkpoint_mismatch")), "failed": 0, "recovered": 0, "skipped": 0, "quarantined": 0}
    ...
    def run_recovery_drill(
        ...
        *,
        ...
        seal_out = self.trigger_seal("shadow_recovery_drill", level="soft", caller_id=caller_id, request_token=request_token)
        ...
        *,
        ...
        def _commit(_lease_id: str) -> Dict[str, Any]:
            ...
            if (
                ...
                self.record_mode("protect", source="shadow:backup_sync", ok=False, error="backup_sync_blocked", metadata=out)
                return {"status": "blocked", "post_state": self._seal.mode(), **out}
            self.backup_dir.mkdir(parents=True, exist_ok=True)
            stamp = self._seal.status().get("last_recovered_at", "") or self._seal.status().get("sealed_at", "") or "now"
            stamp = str(stamp).replace(":", "").replace("-", "")
            snap_dir = self.backup_dir / f"snapshot_{stamp}"
            snap_dir.mkdir(parents=True, exist_ok=True)
            copied = []
            for p in [self.ledger.events_file, self.ledger.checkpoints_file, self._seal.manifest_file]:
                if not p.exists():
                    continue
                dst = snap_dir / p.name
                shutil.copy2(p, dst)
            ...
            out = {"status": "success", "post_state": self._seal.mode(), "backup_dir": str(snap_dir), "copied": copied}
            self.record_mode("protect", source="shadow:backup_sync", ok=True, metadata=out)
            return out
        ...
        *,
        ...
        def _commit(_lease_id: str) -> Dict[str, Any]:
            ...
            if (not src.exists()) or (not self._is_whitelisted_backup_snapshot(src)):
                ...
                self.record_mode("protect", source="shadow:restore_backup_snapshot", ok=False, error="backup_snapshot_not_whitelisted", metadata=out)
                return {"post_state": self._seal.mode(), **out}
            verify = self.ledger.verify_snapshot(str(src))
            if not bool(verify.get("ok", False)):
                out = {"status": "blocked", "reason": "backup_snapshot_invalid", "verify": verify, "path": str(src)}
                self.record_mode("protect", source="shadow:restore_backup_snapshot", ok=False, error="backup_snapshot_invalid", metadata=out)
                return {"post_state": self._seal.mode(), **out}
            ...
            if mf.exists():
                try:
                    ...
                    return {"status": "blocked", "reason": f"manifest_restore_error:{exc}", "post_state": self._seal.mode()}
            ...
            self.record_mode("protect", source="shadow:restore_backup_snapshot", ok=True, metadata=out)
            return out

        return self.gate.execute(
            op_name="restore_backup_snapshot",
            permission="shadow:restore_backup_snapshot",
            req=self._req(caller_id, request_reason=f"restore_backup_snapshot:{backup_events_path}", request_token=request_token),
            pre_state=pre,
            callback=_commit,
            cooldown_s=60,
            ttl_s=180,
    ...
    def restore_shadow_snapshot(
        ...
        *,
        ...
        def _commit(_lease_id: str) -> Dict[str, Any]:
            ...
            if not bool(verify.get("ok", False)):
                return {"status": "blocked", "post_state": self._seal.mode(), "error": str(verify.get("reason", "snapshot_invalid")), "verify": verify}
            ...
            return {"status": "success", "post_state": self._seal.mode(), "restored_from": str(src), "verify": verify, "heal": heal}

        return self.gate.execute(
            op_name="restore_snapshot",
            permission="shadow:restore_snapshot",
            req=self._req(caller_id, request_reason=f"restore_snapshot:{snapshot_path}", request_token=request_token),
            pre_state=pre,
            callback=_commit,
            cooldown_s=60,
            ttl_s=180,
    ...
    def restore_manifest_snapshot(
        ...
        *,
        ...
        def _commit(_lease_id: str) -> Dict[str, Any]:
            ...
            return {"status": "success" if ok else "blocked", "post_state": self._seal.mode(), **out}

        return self.gate.execute(
            op_name="restore_manifest",
            permission="shadow:manifest_restore",
            req=self._req(caller_id, request_reason=f"restore_manifest:{snapshot_path}", request_token=request_token),
            pre_state=pre,
            callback=_commit,
            cooldown_s=60,
            ttl_s=120,
    ...
    def health_check(self, *, readonly: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            report.setdefault("errors", []).append(f"report_persist_error:{exc}")
        return report
