...
class ShadowRecoveryGuard:
    ...
    def __init__(
        ...
        shadow_dir: Path,
        *,
        admission_check: Optional[Callable[[str, Dict[str, Any]], Dict[str, Any]]] = None,
    ) -> None:
        self.ledger = ledger
        ...
        self.quarantine = ShadowQuarantine(self.shadow_dir)
    ...
    def decide(self, rows: Iterable[RecoveryRecord], target: str) -> Tuple[List[RecoveryRecord], List[RecoveryRecord], List[RecoveryRecord]]:
        ...
        for rec in rows:
            ...
            if rec.content_hash and hash_exists_func:
                try:
                    ...
                    pass
            src = rec.source if str(rec.source).startswith(allow_source_prefix) else f"{allow_source_prefix}{rec.source}"
            try:
                write_target(
                    rec.text,
                    src,
    ...
    def quarantine(self, rows: Iterable[RecoveryRecord], reason: str = "recovery_guard") -> Dict[str, Any]:
        ...
        sev = "low"
        for rec in rows:
            out = self.quarantine.append(rec.to_dict(), reason=reason)
            cnt += 1
