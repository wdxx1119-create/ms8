...
from memory.security.shadow import get_shadow_system
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="openclaw-memory shadow CLI")
    ...
    if args.cmd == "token-issue":
        ...
        custom = [
            x.strip()
            for x in str(getattr(args, "permissions", "")).split(",")
            if x.strip()
        ]
