...
from ms8.app.schemas.review_schema import BatchReviewResult
...
class BatchReview:
    ...
    def apply(
        ...
        items = sorted(
            items,
            key=lambda x: (
                self._risk_rank(getattr(x, "risk_level", "medium")),
        ...
        return BatchReviewResult(
            reviewed=len(reviewed_items), accepted=accepted, rejected=rejected, items=reviewed_items
        )
