...
from ms8.engine_core.file_write_guard import atomic_write_json
...
class RuleOptimizer:
    ...
    def optimize_category(self, category: str) -> float:
        ...
        current = self.threshold_manager.cfg.category_thresholds.get(
            category, self.threshold_manager.cfg.global_min_confidence
        )
    ...
    def suggest_threshold_updates(
        ...
        def _sample_count(row: dict[str, object]) -> int:
            value = row.get("samples", 0)
        ...
        payload = {
            ...
            "suggestions": sorted(suggestions, key=_sample_count, reverse=True),
        }
        if output_path:
