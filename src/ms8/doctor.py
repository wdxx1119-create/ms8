...
def _self_check_guidance(check_ids: list[str]) -> list[dict[str, str]]:
    ...
    known = {
        ...
        "m11_project_memory_health": {
            "explanation": "project-memory registry, sqlite/index state, or watch/service state needs attention.",
            "next": "ms8 absorb project-memory status",
        },
        "m12_validation_suite_runtime": {
            "explanation": "validation suite has no executable tests or currently returns a failed runtime status.",
            "next": "ms8 ops validation-suite",
        },
        "m13_session_sync_health": {
            "explanation": "auto-memory session sync state or lock metadata is stale, unreadable, or orphaned.",
            "next": "ms8 ops self-repair-run --mode apply --check-id m13_session_sync_health",
        },
    }
...
def run_doctor() -> int:
    ...
    if self_check_security_degraded or schema_invalid > 0:
    ...
    if str(os.environ.get("MS8_DOCTOR_ALLOW_DEGRADED", "0")).strip().lower() in {"1", "true", "yes", "on"}:
        return 0
    fatal_health_states = {runtime_health, security_governance_health}
    if "degraded" in fatal_health_states:
        return 1
