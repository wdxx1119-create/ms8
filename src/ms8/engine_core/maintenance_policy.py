...
def should_rebalance_feedback(stats: dict[str, Any]) -> bool:
    return float(stats.get("feedback_dominant_ratio", 0.0)) >= float(
        stats.get("feedback_dominant_ratio_threshold", 0.9)
    )
...
def should_auto_navigate_threshold_suggestions(stats: dict[str, Any]) -> bool:
    if not bool(stats.get("threshold_auto_navigate_enabled", False)):
        return False
    pending = int(stats.get("threshold_suggestion_pending_count", 0) or 0)
    if pending <= 0:
        return False
    min_age_minutes = float(stats.get("threshold_suggestion_pending_min_age_minutes", 0.0) or 0.0)
    oldest_age_minutes = float(stats.get("threshold_suggestion_pending_oldest_age_minutes", 0.0) or 0.0)
    return oldest_age_minutes >= max(0.0, min_age_minutes)


def should_batch_extract_kg(stats: dict[str, Any]) -> bool:
    return bool(stats.get("kg_batch_extract_pending_signal", False))


def should_trigger_tiering(stats: dict[str, Any]) -> bool:
    return int(stats.get("tiering_candidate_count", 0) or 0) >= int(stats.get("tiering_candidate_threshold", 1) or 1)
...
def build_policy_actions(stats: dict[str, Any]) -> list[MaintenanceAction]:
    ...
    if should_rebalance_feedback(stats):
        actions.append(
            MaintenanceAction("rebalance_feedback_distribution", "feedback_distribution_skewed", priority=60)
        )
    ...
    if should_batch_extract_kg(stats):
        actions.append(MaintenanceAction("trigger_batch_extract_kg", "kg_pending_source_changes", priority=56))
    if should_trigger_tiering(stats):
        actions.append(MaintenanceAction("trigger_memory_tiering", "tiering_candidates_ready", priority=58))
    ...
    if should_auto_navigate_threshold_suggestions(stats):
        actions.append(
            MaintenanceAction(
                "auto_navigate_threshold_suggestions",
                "threshold_pending_auto_navigation_due",
                priority=66,
            )
        )
...
def gather_policy_stats(workspace_dir: Path, policy_cfg: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    stats: dict[str, Any] = {
        ...
        "threshold_suggestion_pending_count": 0,
        "threshold_suggestion_pending_oldest_age_minutes": 0.0,
        "threshold_suggestion_pending_min_age_minutes": 5.0,
        "kg_batch_extract_pending_signal": False,
        "kg_batch_extract_source_lag_minutes_threshold": 5.0,
        "tiering_candidate_count": 0,
    ...
    stats["threshold_auto_navigate_enabled"] = bool(cfg.get("threshold_auto_navigate_enabled", False))
    ...
    for key in (
        ...
        "threshold_suggestion_pending_min_age_minutes",
        "kg_batch_extract_source_lag_minutes_threshold",
        "tiering_candidate_threshold",
        "tiering_retention_days",
    ...
    if semantic_cache.exists():
        try:
            ...
            print(f"[MaintenancePolicy] Failed reading semantic cache {semantic_cache}: {exc}")
    ...
    if review_file.exists():
        ...
        try:
            ...
            stats["review_backlog_stale_hours"] = max(
                0.0, (datetime.now(timezone.utc) - mtime).total_seconds() / 3600.0
            )
        except OSError:
    ...
    pending_file = memory_dir / "threshold_suggestions_pending.json"
    if pending_file.exists():
        try:
            obj = json.loads(pending_file.read_text(encoding="utf-8"))
            items = obj.get("items", []) if isinstance(obj, dict) else []
            pending_rows = [x for x in items if isinstance(x, dict) and str(x.get("status", "pending")) == "pending"]
            stats["threshold_suggestion_pending_count"] = len(pending_rows)
            oldest_dt = None
            ...
            if oldest_dt is not None:
                stats["threshold_suggestion_pending_oldest_age_minutes"] = max(
                    0.0, (datetime.now(timezone.utc) - oldest_dt).total_seconds() / 60.0
                )
        except (OSError, TypeError, ValueError, json.JSONDecodeError) as exc:
            print(f"[MaintenancePolicy] Failed reading threshold suggestions {pending_file}: {exc}")
    ...
    if verify_file.exists():
        try:
            ...
            print(f"[MaintenancePolicy] Failed reading shadow verify log {verify_file}: {exc}")
    ...
    if backup_manifest.exists():
        try:
            ...
            if isinstance(rows, list) and rows:
                last_row: dict[str, Any] = rows[-1] if isinstance(rows[-1], dict) else {}
                ts_raw = str(last_row.get("ts", "") or "")
                if ts_raw:
            ...
            print(f"[MaintenancePolicy] Failed reading shadow backup manifest {backup_manifest}: {exc}")

    # Count today's auto-seal runs from policy log for daily cap.
    policy_log = memory_dir / "maintenance_policy_log.jsonl"
    last_kg_batch_extract_at = None
    ...
    if policy_log.exists():
        ...
        for ln in policy_log.read_text(encoding="utf-8", errors="ignore").splitlines()[-3000:]:
            ...
            try:
                ...
                continue
            if act == "shadow_auto_seal" and dt.date().isoformat() == today_iso:
                auto_seal_today += 1
            if act == "trigger_batch_extract_kg":
                if last_kg_batch_extract_at is None or dt > last_kg_batch_extract_at:
                    last_kg_batch_extract_at = dt
            if act == "trigger_memory_tiering":
                if last_tiering_at is None or dt > last_tiering_at:
                    last_tiering_at = dt
    ...
    source_candidates = [memory_md, memory_dir / "memory_blocks.json", memory_dir / "auto_memory_records.jsonl"]
    source_candidates.extend(list_daily_log_files(memory_dir))
    for path in source_candidates:
        try:
            if not path.exists():
                continue
            dt = datetime.fromtimestamp(path.stat().st_mtime, tz=timezone.utc)
            if latest_source_mtime is None or dt > latest_source_mtime:
                latest_source_mtime = dt
        except OSError:
            continue
    if latest_source_mtime is not None:
        lag_minutes = float(stats.get("kg_batch_extract_source_lag_minutes_threshold", 5.0) or 5.0)
        if last_kg_batch_extract_at is None:
            stats["kg_batch_extract_pending_signal"] = True
        else:
            delta_minutes = (latest_source_mtime - last_kg_batch_extract_at).total_seconds() / 60.0
            stats["kg_batch_extract_pending_signal"] = delta_minutes >= max(0.0, lag_minutes)

    retention_days = int(stats.get("tiering_retention_days", 7) or 7)
    today_date = datetime.now(timezone.utc).date()
    tiering_candidates = 0
    for log_file in list_daily_log_files(memory_dir):
        try:
            log_date = datetime.fromisoformat("-".join(log_file.stem.split("-")[0:3])).date()
        except ValueError:
            continue
        if (today_date - log_date).days > retention_days:
            tiering_candidates += 1
    stats["tiering_candidate_count"] = tiering_candidates
    ...
    if latest.exists():
        try:
            ...
            print(f"[MaintenancePolicy] Failed reading self-check latest {latest}: {exc}")
    ...
    try:
        ...
        if history_dir.exists():
            for hp in sorted(history_dir.glob("*.json"), reverse=True):
                ...
                if latest_l1_minutes is not None and latest_l2l3_minutes is not None and latest_l4_minutes is not None:
                    break
        ...
        print(f"[MaintenancePolicy] Failed reading self-check history: {exc}")
    ...
    try:
        ...
        print(f"[MaintenancePolicy] Failed reading alerts bridge stats: {exc}")
