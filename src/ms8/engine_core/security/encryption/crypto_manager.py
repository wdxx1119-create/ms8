...
class CryptoManager:
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self._last_unlock_at: str | None = None
    ...
    def encrypt_before_write(self, data: bytes, file_type: str, target_path: Path | None = None) -> bytes:
        if not self.is_enabled():
            return data
        ...
        return encrypt_bytes(
            ...
            file_type=file_type,
            dek=self._dek or b"",
            kdf=self.km.load_material().get("kdf", {}).get("kdf", "argon2id"),
        )

    def decrypt_after_read(self, blob: bytes, target_path: Path | None = None, allow_plaintext: bool = True) -> bytes:
        if not self.is_enabled():
            return blob
        path = Path(target_path) if target_path else None
        if path is not None and not self.should_protect_path(path):
