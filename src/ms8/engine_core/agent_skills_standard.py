...
class SkillResource:
    ...
    def to_dict(self) -> dict:
        return {"name": self.name, "content": self.content, "type": self.type}
        ...
        for resource_file in resources_dir.glob("*"):
            if resource_file.is_file():
                try:
                    ...
                    self.resources.append(SkillResource(name=resource_file.name, content=content, type=resource_type))
    ...
    def _load_examples(self) -> None:
        ...
        for example_file in examples_dir.glob("*.md"):
            try:
                ...
                self.examples.append({"name": example_file.stem, "content": content, "file": str(example_file)})
            except (OSError, RuntimeError, TypeError, ValueError) as e:
    ...
    def save(self) -> bool:
        ...
        if self.tests:
            ...
            with open(tests_file, "w", encoding="utf-8") as f:
                json.dump({"tests": [test.to_dict() for test in self.tests]}, f, indent=2)
        ...
        for trigger in self.metadata.triggers:
            ...
            if trigger.startswith("/") and trigger.endswith("/"):
                try:
                    ...
                    print(f"[AgentSkill] Invalid trigger regex '{trigger}' for skill '{self.path}': {exc}")
