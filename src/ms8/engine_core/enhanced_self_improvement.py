...
class EnhancedSelfImprovement(SelfImprovementEngine):
    ...
    def __init__(self, memory_core, config: LLMConfig | None = None):
        """
        初始化增强版自我改进引擎
    ...
    def _parse_llm_reason(self, response: str) -> dict:
        ...
        for line in lines:
            if ":" in line:
                ...
                if key == "建议更新":
                    # 映射到英文块名
                    mapping = {"human": "human", "persona": "persona", "archival": "archival"}
    ...
    def _detect_patterns_simple(self, conversations: list[dict]) -> dict:
        ...
        return {"word_frequency": sorted_words[:20], "total_conversations": len(conversations)}
