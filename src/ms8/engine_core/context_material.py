...
def build_candidate_profiles(
    ...
    for idx, c in enumerate(candidates):
        ...
        profile = {
            ...
            "pronoun_resolution_confidence": _safe_float(
                context_signals.get("pronoun_resolution_confidence", 0.0), 0.0
            ),
        }
...
def _query_complexity(query: str) -> dict[str, Any]:
    ...
    score = min(
        1.0,
        0.1 * min(6, token_len) + (0.32 if has_complex_cue else 0.0) + (0.2 if multi_clause else 0.0),
    )
...
def project_response_context(material: dict[str, Any]) -> dict[str, Any]:
    ...
    high_value = [p for p in profiles if p.get("injectability", False) and str(p.get("trust", "")) in HIGH_TRUST_LEVELS]
    high_value.sort(
        key=lambda x: (
            x.get("query_coverage", 0.0),
            x.get("density_score", 0.0),
            x.get("recency_score", 0.0),
        ),
        reverse=True,
    )
    return {
