...
class GitHubSkillDiscovery:
    ...
    def _local_skill_fallback(self, query: str | None = None, limit: int = 20) -> list[dict]:
    ...
    def search_skills(
        ...
        for repo in self.skill_repos:
            try:
                repo_skills = self._search_repo_skills(repo, query, category, tags, min_stars)
        ...
        if len(all_skills) < max(3, min(limit, 5)):
            try:
                ...
                logger.warning("Error searching GitHub code: %s", e)
    ...
    def _search_github_code(self, query: str | None = None, limit: int = 20) -> list[dict]:
        ...
        if response is None or response.status_code != 200:
            if response is not None:
                logger.warning("GitHub code search failed: status=%s", response.status_code)
            return []
    ...
    def _search_repo_skills(
        ...
        if repo_response.status_code != 200:
            logger.warning("Failed to get repo %s: status=%s", repo, repo_response.status_code)
            return []
        ...
        try:
            ...
            if contents_response.status_code == 200:
                ...
                for item in contents:
                    if item["type"] == "dir":
                        skill_meta = self._get_skill_metadata(owner, repo_name, item["path"], repo_info)
                        if skill_meta:
                            skills.append(skill_meta)

            # Also check root level for SKILL.md files
            if not skills:
                root_contents_response = self._request(
                    f"{self.base_url}/repos/{owner}/{repo_name}/contents",
                )

                if root_contents_response is not None and root_contents_response.status_code == 200:
                    contents = root_contents_response.json()
        ...
        except (requests.RequestException, ValueError, TypeError, KeyError, json.JSONDecodeError) as e:
            logger.warning("Error getting contents from %s: %s", repo, e)

        return skills

    def _get_skill_metadata(self, owner: str, repo_name: str, path: str, repo_info: dict) -> dict | None:
        ...
        try:
            ...
            logger.warning("Error getting skill metadata: %s", e)
