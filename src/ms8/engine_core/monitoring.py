...
except ImportError:  # pragma: no cover - standalone import compatibility for spec_from_file_location tests
    from ms8.engine_core.file_write_guard import (
        atomic_write_json,
        atomic_write_text,
        guarded_file_write,
    )
    from ms8.engine_core.metrics_contract import core_metric_snapshot, metric_contract
...
class MemoryMonitoring:
    ...
    def _tail_jsonl(self, path: Path, limit: int = 50) -> list[dict]:
        ...
        for line in path.read_text(encoding="utf-8").splitlines()[-limit:]:
            ...
            try:
                ...
                logger.debug("monitoring: skip invalid jsonl row in %s: %s", path, exc)
                continue
        return rows
    ...
    def _alert_with_cooldown(
        self, code: str, message: str, severity: str, cooldown_hours: int, extra: dict | None = None
    ) -> dict | None:
    ...
    def _load_auto_entries(self) -> list[dict]:
        ...
        try:
            ...
            logger.debug("monitoring: failed to load auto memory log %s: %s", auto_log, exc)
            return []
    ...
    def _self_check_stats(self) -> dict[str, Any]:
        ...
        stale_threshold_hours = float(
            mon_alerts.get("self_check_stale_hours", cfg.get("watchdog_interval_hours", 2)) or 2
        )
        ...
        if latest.exists():
            try:
                ...
                logger.debug("monitoring: failed to parse self-check latest json: %s", latest)
        ...
        if heartbeat_path.exists():
            try:
                ...
                logger.debug("monitoring: failed to stat self-check heartbeat: %s", heartbeat_path)

        age_candidates = [
            x
            for x in [stats.get("latest_age_minutes"), stats.get("heartbeat_age_minutes")]
    ...
    def _threshold_suggestion_stats(self) -> dict[str, Any]:
        ...
        if pending_file.exists():
            try:
                ...
                logger.debug("monitoring: failed to parse threshold suggestion file: %s", pending_file)
    ...
    def _compression_freshness(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.debug("monitoring: failed to parse compression report: %s", last)
    ...
    def _shadow_runtime_stats(self) -> dict[str, Any]:
        ...
        if manifest.exists():
            try:
                ...
                logger.debug("monitoring: failed to parse shadow manifest: %s", manifest)
    ...
    def _shadow_startup_integrity_stats(self, window_hours: int = 24, limit: int = 5000) -> dict[str, Any]:
        ...
        def _sig_count(item: dict[str, Any]) -> float:
            value = item.get("count")
            if isinstance(value, (int, float)):
                return float(value)
            return 0.0

        top_signatures: list[dict[str, Any]] = sorted(
    ...
    def _compute_rates_v2(self, auto_entries: list[dict]) -> dict:
        """
        Preview metrics for the new governance-oriented capture model.
        Keeps old capture_rate intact while providing a more trustworthy quality lens.
        """
        def _is_real_auto_entry(e: dict) -> bool:
            status = str(e.get("status", ""))
            source = str(e.get("source", ""))
        ...
        def _is_high_value(e: dict) -> bool:
            ...
            txt = _entry_text(e).lower()
            if cat in {"preference", "decision", "constraint", "feedback", "project", "identity"}:
                return True
            kws = (
                "偏好",
                "喜欢",
        ...
        def _is_low_value(e: dict) -> bool:
            txt = _entry_text(e).lower()
            if len(txt) <= 8:
                return True
            low_tokens = ("ok", "好的", "同意", "收到", "嗯", "知道了", "thanks", "thx")
            return txt in low_tokens

        def _is_pipeline_success(e: dict) -> bool:
            s = str(e.get("status", "")).strip().lower()
            return s not in {"error", "failed", "exception"}

        def _is_admitted_or_pending(e: dict) -> bool:
            s = str(e.get("status", "")).strip().lower()
            if s in {"success", "accepted", "review", "pending_review", "pending"}:
                return True
            route = str(e.get("route", "")).strip().lower()
            return route in {"accepted", "pending_review"}

        def _is_rejected(e: dict) -> bool:
            s = str(e.get("status", "")).strip().lower()
            if s in {"rejected", "dropped", "blocked"}:
                return True
            dropped = e.get("dropped", [])
            return isinstance(dropped, list) and len(dropped) > 0

        real_auto = [e for e in auto_entries if _is_real_auto_entry(e)]
        eligible_events = len(real_auto)
        accepted = sum(1 for e in real_auto if _is_admitted_or_pending(e))
        pipeline_success = sum(1 for e in real_auto if _is_pipeline_success(e))

        high_value_events = [e for e in real_auto if _is_high_value(e)]
        high_value_total = len(high_value_events)
        high_value_captured = sum(1 for e in high_value_events if _is_admitted_or_pending(e))

        low_value_events = [e for e in real_auto if _is_low_value(e)]
        low_value_total = len(low_value_events)
        low_value_rejected = sum(1 for e in low_value_events if _is_rejected(e))

        def _rate(num: int, den: int) -> float:
            return round(num / den, 4) if den > 0 else 1.0

        return {
            "eligible_event_rate": _rate(eligible_events, max(1, eligible_events)),  # informational, kept as 1.0 when sampled
            "admission_accept_rate": _rate(accepted, eligible_events),
            "pipeline_success_rate": _rate(pipeline_success, eligible_events),
            "valuable_capture_rate": _rate(high_value_captured, high_value_total),
            "noise_block_rate": _rate(low_value_rejected, low_value_total),
            "eligible_events": eligible_events,
            "accepted_events": accepted,
            "pipeline_success_events": pipeline_success,
            "high_value_events": high_value_total,
            "high_value_captured_events": high_value_captured,
            "low_value_events": low_value_total,
            "low_value_rejected_events": low_value_rejected,
        }

    def _evaluate_slo_v2_preview(self, rates_v2: dict) -> dict:
        """
        Preview-only SLO evaluation for new metrics.
        Does not replace the existing SLO gate in current phase.
        """
        mon_cfg = self.config["settings"]["memory"].get("monitoring", {})
        slo = mon_cfg.get("slo_v2_preview", {})
        pipeline_min = float(slo.get("pipeline_success_rate_min", 0.98))
        valuable_min = float(slo.get("valuable_capture_rate_min", 0.85))
        noise_min = float(slo.get("noise_block_rate_min", 0.90))

        checks = {
            "pipeline_success_rate": float(rates_v2.get("pipeline_success_rate", 0.0)) >= pipeline_min,
            "valuable_capture_rate": float(rates_v2.get("valuable_capture_rate", 0.0)) >= valuable_min,
            "noise_block_rate": float(rates_v2.get("noise_block_rate", 0.0)) >= noise_min,
        }
        return {
            "targets": {
                "pipeline_success_rate_min": pipeline_min,
                "valuable_capture_rate_min": valuable_min,
                "noise_block_rate_min": noise_min,
            },
            "checks": checks,
            "all_ok": all(checks.values()),
            "phase": "preview_dual_track",
        }
    ...
    def _evaluate_slo(self, rates: dict, shadow_stats: dict[str, Any], maintenance_stats: dict[str, Any]) -> dict:
        ...
        capture_samples_min = int(slo.get("capture_rate_min_samples", 30) or 30)
        injection_samples_min = int(slo.get("injection_rate_min_samples", 10) or 10)
        ...
        sr = (
            maintenance_stats.get("shadow_replay", {})
            if isinstance(maintenance_stats.get("shadow_replay", {}), dict)
            else {}
        )
        ...
        injection_events = int(rates.get("injection_events", 0) or 0)
        capture_under_sample = auto_total_entries < max(1, capture_samples_min)
        injection_under_sample = injection_events < max(1, injection_samples_min)
        checks = {
            "capture_rate": capture_under_sample or rates.get("capture_rate", 0.0) >= capture_min,
            "injection_rate": injection_under_sample or rates.get("injection_rate", 0.0) >= injection_min,
        ...
        return {
            "targets": {
                ...
                "capture_rate_min_samples": capture_samples_min,
                "injection_rate_min": injection_min,
                "injection_rate_min_samples": injection_samples_min,
            ...
            "actuals": {
                ...
                "capture_rate_under_sample": capture_under_sample,
                "injection_rate_under_sample": injection_under_sample,
            },
            "checks": checks,
            "all_ok": all(checks.values()),
        }
    ...
    def _detect_anomalies(self, snapshot: dict[str, Any], auto_entries: list[dict]) -> list[dict]:
        ...
        if not bool(checks.get("capture_rate", True)):
            alert = self._alert_with_cooldown(
                ...
                extra={
                    ...
                    "target": slo.get("targets", {}).get("capture_rate_min"),
                },
            )
            if alert:
                emitted.append(alert)
        if not bool(checks.get("injection_rate", True)):
            alert = self._alert_with_cooldown(
                ...
                extra={
                    ...
                    "target": slo.get("targets", {}).get("injection_rate_min"),
                },
            )
            if alert:
                emitted.append(alert)
        ...
        review_stats = (
            snapshot.get("review_backlog", {}) if isinstance(snapshot.get("review_backlog", {}), dict) else {}
        )
        ...
        comp = (
            snapshot.get("compression_freshness", {})
            if isinstance(snapshot.get("compression_freshness", {}), dict)
            else {}
        )
        ...
        if isinstance(stale_hours, (int, float)) and stale_hours >= stale_th:
            alert = self._alert_with_cooldown(
                ...
                cooldown_hours=cooldown,
                extra={
                    "hours_since_last": stale_hours,
                    "threshold": stale_th,
                    "last_report": comp.get("last_report", ""),
                },
            )
            if alert:
                emitted.append(alert)
        ...
        mp = (
            snapshot.get("maintenance_policy_stats", {})
            if isinstance(snapshot.get("maintenance_policy_stats", {}), dict)
            else {}
        )
        ...
        if replay_remaining >= replay_remaining_th:
            ...
            if alert:
                emitted.append(alert)
        shadow_stats = (
            snapshot.get("shadow_runtime_stats", {})
            if isinstance(snapshot.get("shadow_runtime_stats", {}), dict)
            else {}
        )
        ...
        startup_integrity = (
            snapshot.get("shadow_startup_integrity", {})
            if isinstance(snapshot.get("shadow_startup_integrity", {}), dict)
            else {}
        )
    ...
    def write_daily_health_report(self, snapshot: dict[str, Any]) -> dict[str, str]:
        ...
        slo = snapshot.get("slo", {})
        rates = snapshot.get("rates", {})
        rates_v2 = snapshot.get("rates_v2", {}) if isinstance(snapshot.get("rates_v2", {}), dict) else {}
        lines = [
            ...
            f"- pipeline_success_rate_v2: {rates_v2.get('pipeline_success_rate', 0.0)}",
            f"- valuable_capture_rate_v2: {rates_v2.get('valuable_capture_rate', 0.0)}",
            f"- noise_block_rate_v2: {rates_v2.get('noise_block_rate', 0.0)}",
    ...
    def status(self, persist_reports: bool = True) -> dict[str, Any]:
        mon_cfg = self.config["settings"]["memory"].get("monitoring", {})
        ...
        rates_v2 = self._compute_rates_v2(auto_entries)
        ...
        snapshot = {
            ...
            "subagent_log_files": len(list((self.memory_dir / "subagent_logs").glob("*.log")))
            if (self.memory_dir / "subagent_logs").exists()
            else 0,
        }
        ...
        snapshot["slo_v2_preview"] = self._evaluate_slo_v2_preview(rates_v2)
        ...
        if persist_reports:
            ...
            snapshot["alerts_emitted"] = []
            snapshot["alerts_recent"] = self._tail_jsonl(self._alert_log_path(), limit=20)
            snapshot["report_paths"] = {}
        return snapshot
