...
class ImprovementPlan:
    ...
    actual_improvement: float | None  # 实际提升
    completed: datetime | None  # 完成时间
...
class MetaCognitionSystem:
    ...
    def __init__(self, llm: _ChatLLM | None = None, config: LLMConfig | None = None):
        """
        初始化元认知系统
        ...
        """
        try:
            self.llm: _ChatLLM | None = llm or LocalLLM(config or LLMConfig())
    ...
    def _release_lock(self) -> None:
        try:
            ...
            print(f"[MetaCognition] Failed releasing lock {self.lock_file}: {exc}")
    ...
    def _load_meta_data(self) -> None:
        ...
        try:
            ...
            self.reports = {k: PerformanceReport.from_dict(v) for k, v in data.get("reports", {}).items()}
            self.improvement_plans = {
                k: ImprovementPlan.from_dict(v) for k, v in data.get("improvement_plans", {}).items()
            }
            self.assessments = {k: SelfAssessment.from_dict(v) for k, v in data.get("assessments", {}).items()}
        ...
        try:
            ...
            if self.meta_file.exists():
                ...
                try:
                    ...
                    print(f"[MetaCognition] Failed rotating backup file {backup_path}: {exc}")
    ...
    async def self_monitor(self, conversations: list[dict], period: str = "daily") -> PerformanceReport:
        """
        自我监控 - 评估当前性能
    ...
    async def _evaluate_response_quality(self, conversations: list[dict]) -> float:
        ...
        # 获取 AI 响应
        ai_responses = [c.get("content", "") for c in conversations if c.get("role") == "assistant"][-10:]
...
"""
        ...
        if self._llm_allowed():
            try:
                ...
                response = await self._chat(messages, temperature=0.3, max_tokens=50)
                import re
    ...
    def _calculate_response_speed(self, conversations: list[dict]) -> float:
        ...
        if len(timestamps) < 2:
            return 0.8
        try:
            parsed = [datetime.fromisoformat(str(ts)) for ts in timestamps]
...
"""
    ...
    def _calculate_task_completion(self, conversations: list[dict]) -> float:
        ...
        hits = sum(
            ...
            if c.get("role") == "assistant" and any(t in c.get("content", "") for t in done_tokens)
        )
    ...
    async def _identify_strengths(self, metrics: dict[str, float], conversations: list[dict]) -> list[str]:
        """识别优势"""
        strengths = []
        ...
        if strengths and self._llm_allowed():
...
"""
            ...
            try:
                messages = [{"role": "user", "content": prompt}]
                response = await self._chat(messages, temperature=0.5, max_tokens=300)
    ...
    async def _identify_weaknesses(self, metrics: dict[str, float], conversations: list[dict]) -> list[str]:
        """识别弱点"""
        weaknesses = []
...
"""
            try:
                messages = [{"role": "user", "content": prompt}]
                response = await self._chat(messages, temperature=0.5, max_tokens=500)
                llm_weaknesses = [
...
"""
    ...
    async def create_improvement_plan(
        self, area: ImprovementArea, description: str, priority: int = 3
    ) -> ImprovementPlan:
        """
        创建改进计划
    ...
    async def _generate_improvement_actions(self, area: ImprovementArea, description: str) -> list[str]:
        """生成改进行动"""
...
"""
    ...
    async def _estimate_improvement(self, area: ImprovementArea, actions: list[str]) -> float:
        """估计改进效果"""
...
"""
        ...
        try:
            ...
            print(f"[MetaCognition] Improvement estimate fallback triggered: {exc}")
    ...
    def get_performance_trend(self, days: int = 7) -> dict:
        ...
        if not recent_metrics:
            return {"trend": "stable", "change": 0, "data_points": 0}

        # 计算趋势
        scores = [m["overall_score"] for m in recent_metrics]

        if len(scores) < 2:
            return {"trend": "stable", "change": 0, "data_points": len(scores)}
