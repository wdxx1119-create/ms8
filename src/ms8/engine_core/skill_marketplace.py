...
from .agent_skills_standard import AgentSkill
    ...
    def _load_registries(self) -> list[dict]:
        ...
        if self.registry_file.exists():
            try:
                ...
                print(f"[SkillMarketplace] Failed loading registries from {self.registry_file}: {exc}")
...
class SkillInstaller:
    ...
    def install_from_registry(self, skill_id: str, scope: str = "project") -> dict:
        ...
        try:
            ...
            return {"status": "error", "error": f'No registry found for author "{author}"'}
    ...
    def install_from_file(self, file_path: str, scope: str = "project") -> dict:
        ...
        try:
            ...
            if not path.exists():
                return {"status": "error", "error": f"Path does not exist: {file_path}"}
            ...
            if not skill.load():
                return {"status": "error", "error": "Failed to load skill"}

            # Copy to appropriate directory
            metadata = skill.metadata
            if metadata is None:
                return {"status": "error", "error": "Skill metadata missing"}
            target_dir = self.skill_manager.skill_dirs.get(scope, Path(".")) / metadata.name
            ...
            return {
                "status": "success",
                "message": f'Installed skill "{metadata.name}"',
                "source": "file",
                "path": str(file_path),
                "scope": scope,
    ...
    def uninstall(self, skill_name: str, scope: str = "project") -> dict:
        ...
        try:
            ...
            return {"status": "success", "message": f'Uninstalled skill "{skill_name}"'}
