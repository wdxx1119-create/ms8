...
class AutoMemoryExtractor:
    ...
    def __init__(self, memory_core: Any) -> None:
        ...
        sessions_dir_raw = str(
            session_cfg.get("sessions_dir", str(Path.home() / ".openclaw" / "agents" / "main" / "sessions"))
        )
        self.session_dir = Path(sessions_dir_raw).expanduser()
        self.session_dirs_glob = str(
            session_cfg.get("sessions_dirs_glob", str(Path.home() / ".openclaw" / "agents" / "*" / "sessions"))
        )
        ...
        if build_pipeline is not None:
            try:
                self.pipeline = build_pipeline(
                    memory_core.config["workspace_dir"],
                    memory_core.config["settings"].get("memory", {}),
                )
            except (AttributeError, OSError, RuntimeError, TypeError, ValueError) as exc:  # fallback to disabled pipeline
                self._save_log(
                    {
        ...
        try:
            ...
            if acquired:
                try:
                    ...
                    logger.debug("Failed to remove session sync lock dir: %s", self.session_lock_dir)
    ...
    def sync_openclaw_sessions(self, force: bool = False, max_messages: int | None = None) -> dict[str, Any]:
    ...
    def process_interaction(self, text: str, source: str = "interaction") -> list[dict[str, Any]]:
        ...
        if not self.pipeline:
            ...
            if hasattr(self.memory_core, "remember"):
                try:
                    fallback_status = self.memory_core.remember(
                    ...
                    print(f"[AutoMemory] Fallback remember failed for source={source}: {exc}")
            self._save_log(
                {
                    "timestamp": datetime.now().isoformat(),
                    "source": source,
