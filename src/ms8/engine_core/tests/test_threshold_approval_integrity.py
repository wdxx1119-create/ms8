...
def _build_core(tmp_path):
        ...
        "settings": {
            "memory": {
                ...
                "maintenance_policy": {
                    ...
                    "threshold_auto_navigate_auto_reject_failed_guardrail": True,
                },
            }
        },
    }
    ...
    core._threshold_approval_log_file = core.config["memory_dir"] / "threshold_suggestions_approval_log.jsonl"
...
def test_threshold_pending_signature_detects_tamper(tmp_path):
    ...
    payload = {
        "items": [{"approval_id": "a1", "status": "pending"}],
        "last_generated_at": "x",
...
def test_apply_threshold_suggestions_allowlist_and_missing_key_guard(tmp_path):
    ...
    out = core._apply_threshold_suggestions_to_workspace_config(
        [
            ...
            {
                "key": "knowledge_control.retrieval_mix_balancer.unknown_ratio",
                "direction": "increase",
                "delta": 0.1,
            },
        ]
...
def test_auto_navigate_threshold_suggestions_guardrail_blocks_and_rejects(tmp_path):
    core = _build_core(tmp_path)
    core.config["memory_dir"].mkdir(parents=True, exist_ok=True)
    core._save_threshold_pending(
        {
            "items": [
                {
                    "approval_id": "a-block",
    ...
    out = core.auto_navigate_threshold_suggestions(limit=1)
    assert out["status"] == "success"
    assert out["processed_count"] == 1
    items = core.list_pending_threshold_suggestions(include_processed=True)["items"]
    assert str(items[0].get("status")) == "rejected"


def test_auto_navigate_threshold_suggestions_applies_safe_item(tmp_path):
    core = _build_core(tmp_path)
    core.config["memory_dir"].mkdir(parents=True, exist_ok=True)
    (tmp_path / "config.yaml").write_text(
        "memory:\n"
    ...
    items = core.list_pending_threshold_suggestions(include_processed=True)["items"]
    assert str(items[0].get("status")) == "approved"
