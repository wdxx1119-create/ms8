...
class SyntheticMemoryTests(unittest.TestCase):
    ...
    def test_preview_and_rollback_auto_approved(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            core = _FakeCore(Path(tmp))
            synthesizer = MemorySynthesizer(core)
            state = synthesizer._load_state()
            state["candidates"] = [
                {
                    "candidate_id": "c1",
                    "status": "accepted",
            ...
            synthesizer._save_state(state)
            preview = synthesizer.preview_rollback_auto_approved(since_hours=1)
            self.assertEqual(preview.get("count"), 1)
            out = synthesizer.rollback_auto_approved(since_hours=1)
            self.assertEqual(out.get("rolled_back"), 1)
            latest = synthesizer._load_state()["candidates"][0]
            self.assertEqual(latest.get("status"), "revoked")
