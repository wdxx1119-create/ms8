    ...
    def _load(self) -> dict:
        if self.file.exists():
            try:
                ...
                print(f"[Governance] Failed loading report {self.file}: {exc}")
        ...
        for block_name, block_content in combined_sources.items():
            ...
            if subject and subject in block_norm:
                ...
                if negative_polarity and block_positive:
                    conflicts.append(block_name)
                if ("always" in normalized and "never" in block_norm) or (
                    "never" in normalized and "always" in block_norm
                ):
                    conflicts.append(block_name)
            overlap = self._semantic_overlap(content, block_content)
            if overlap >= self.semantic_overlap_threshold and (
                (positive_polarity and block_negative) or (negative_polarity and block_positive)
            ):
                conflicts.append(block_name)
            if (
                overlap >= self.semantic_overlap_threshold
                and block_name not in duplicate_in
                and block_name not in conflicts
            ):
