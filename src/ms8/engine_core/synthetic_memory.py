...
class MemorySynthesizer:
    ...
    def __init__(self, memory_core: Any) -> None:
        ...
        self.auto_confirm_high_conf = float(self.settings.get("auto_confirm_high_confidence", 0.90))
        self.auto_confirm_medium_conf = float(self.settings.get("auto_confirm_medium_confidence", 0.75))
        self.auto_confirm_high_risk_categories = {
            str(x).strip().lower() for x in self.settings.get("auto_confirm_high_risk_categories", [])
        }
        self.auto_confirm_high_risk_keywords = [
            str(x).strip().lower() for x in self.settings.get("auto_confirm_high_risk_keywords", [])
        ]
        ...
        self.auto_approval_rollback_keep_hours = int(self.settings.get("auto_approval_rollback_keep_hours", 72))
        ...
        if not self.history_file.exists():
            with guarded_file_write(self.history_file):
                self.history_file.write_text(
                    json.dumps(
                        {
                            "accepted_struct_keys": [],
                            "rejected_struct_keys": [],
                            "last_updated_at": "",
                        },
                        indent=2,
                        ensure_ascii=False,
        ...
        self.auto_approval_audit_file = self.memory_dir / "synthetic_auto_approval_audit.jsonl"
        self.auto_approval_audit_file.touch(exist_ok=True)
    ...
    def _migrate_legacy_statuses(self) -> None:
        ...
        for item in state.get("candidates", []):
            ...
            if status in {"pending"} or (
                not self.reasoning_only_mode and status in {"candidate_reasoning_only", "promotion_ready"}
            ):
                item["status"] = "review"
                changed = True
            ...
            if "timestamp" not in item:
                ...
                changed = True
            if (
                "knowledge_tier" not in item
                or "trust_level" not in item
                or "admission_decision" not in item
                or "usage_permission" not in item
                or "promotion_state" not in item
            ):
                ...
                item["usage_permission"] = dict(
                    ctl.get(
                        "usage_permission",
                        item.get("usage_permission", {"recall": True, "inject": "weak", "speak": "hint"}),
                    )
                )
    ...
    def _struct_key(self, subject: str, relation_type: str, object_name: str) -> str:
        return (
            f"{self._normalize_entity(subject)}|"
            f"{self._normalize_relation(relation_type)}|"
            f"{self._normalize_entity(object_name)}"
        )
    ...
    def _triage_status(self, total_score: float, evidence: dict[str, Any]) -> str:
        ...
        if (
            self.special_reasoning_enabled
            and int(evidence.get("inference_depth", 1)) >= 2
            and bool(self.settings.get("force_two_hop_review", True))
        ):
            return "review"
    ...
    def _append_to_memory_md(self, candidate: dict[str, Any]) -> None:
        ...
        line = (
            ...
            f"{candidate.get('subject')} "
            f"{self._relation_label(str(candidate.get('relation_type')))} "
    ...
    def _write_back_to_graph(self, candidate: dict[str, Any]) -> dict[str, Any] | None:
        graph = self.memory_core.knowledge_graph
        if not graph:
            return None
    ...
    def _route_candidate_write(self, candidate: dict[str, Any]) -> dict[str, Any]:
        ...
        if tier in {"short_term", "observation"}:
            try:
                ...
                print(f"[SyntheticMemory] Working memory append failed: {exc}")
        return out
    ...
    def _accept_candidate(self, item: dict[str, Any], history: dict[str, Any]) -> None:
        self._accept_candidate_with_meta(item, history, auto_approved=False, policy="")

    def _append_auto_approval_audit(self, payload: dict[str, Any]) -> None:
        row = {"timestamp": datetime.now(timezone.utc).isoformat(), **payload}
        with guarded_file_write(self.auto_approval_audit_file):
            with self.auto_approval_audit_file.open("a", encoding="utf-8") as f:
                f.write(json.dumps(row, ensure_ascii=False) + "\n")

    def _classify_risk(self, relation_type: str, statement: str) -> tuple[bool, str]:
        rel = str(relation_type or "").strip().lower()
        text = str(statement or "").strip().lower()
        if rel in {"contradicts", "causes"} and "risk" in text:
            return True, "security"
        for category in self.auto_confirm_high_risk_categories:
            if category and category in text:
                return True, category
        for kw in self.auto_confirm_high_risk_keywords:
            if kw and kw in text:
                if kw in {"token", "api key", "credential", "password", "密钥"}:
                    return True, "permission"
                if kw in {"决定", "决策", "优先级", "取舍", "should", "must"}:
                    return True, "decision"
                return True, "security"
        return False, "low_risk"

    def _triage_auto_confirmation(self, candidate: dict[str, Any]) -> dict[str, Any]:
        confidence = float(candidate.get("confidence", 0.0) or 0.0)
        statement = str(candidate.get("statement", "") or "")
        relation_type = str(candidate.get("relation_type", "") or "")
        high_risk, risk_category = self._classify_risk(relation_type, statement)
        if high_risk:
            return {
                "tier": "manual_required",
                "action": "review",
                "reason": f"high_risk:{risk_category}",
                "risk_category": risk_category,
                "confidence": confidence,
            }
        if confidence >= self.auto_confirm_high_conf:
            return {
                "tier": "high_conf_auto",
                "action": "auto_accept",
        ...
        if confidence >= self.auto_confirm_medium_conf:
            return {
                "tier": "medium_conf_review",
                "action": "review",
                "reason": "medium_confidence_batch_review",
                "risk_category": "low_risk",
                "confidence": confidence,
            }
        return {
    ...
    def _accept_candidate_with_meta(
        self,
        item: dict[str, Any],
        ...
        *,
        ...
        item.update(ctl)
        if (
            str(item.get("admission_decision", "hold")) != "admit"
            or str(item.get("knowledge_tier", "observation")) == "rejected"
        ):
        ...
        if auto_approved:
            ...
            item["auto_approved_at"] = item["accepted_at"]
            item["auto_approval_policy_version"] = policy or "v1"
            item["review_note"] = (
                "auto_accepted"
                if not item.get("review_note")
                else f"{item.get('review_note')}|auto_accepted"
            )
        else:
            item["auto_approved"] = bool(item.get("auto_approved", False))
            item["review_note"] = "manual_accepted" if not item.get("review_note") else item.get("review_note")
        route = self._route_candidate_write(item)
        if auto_approved:
            batch_id = datetime.now(timezone.utc).strftime("auto-%Y%m%d%H")
            item["auto_approval_batch_id"] = batch_id
            route_rel_id = ""
            try:
                route_rel_id = str((route.get("graph_relation", {}) or {}).get("id", ""))
            except (AttributeError, TypeError, ValueError):
                route_rel_id = ""
            self._append_auto_approval_audit(
                {
                    "candidate_id": str(item.get("candidate_id", "")),
                    "accepted_at": str(item.get("accepted_at", "")),
                    "auto_approval_policy_version": str(item.get("auto_approval_policy_version", "")),
                    "auto_approval_batch_id": batch_id,
                    "risk_category": str(item.get("risk_category", "low_risk")),
                    "confidence": float(item.get("confidence", 0.0) or 0.0),
                    "source_candidate_id": str(item.get("candidate_id", "")),
                    "graph_relation_id": route_rel_id,
    ...
    def rebalance_review_queue(self, max_auto_accept: int = 40, apply_writeback: bool = False) -> dict[str, int]:
        ...
        if apply_writeback and accepted > 0:
            try:
                ...
                print(f"[SyntheticMemory] Reindex failed after rebalance: {exc}")
                ...
                inferred.append(
                    {
                        ...
                        "strength": round(
                            min(float(r1.get("strength", 0.6)), float(r2.get("strength", 0.6))) * 0.85,
                            3,
                        ),
                        "confidence": round(
                            min(float(r1.get("confidence", 0.6)), float(r2.get("confidence", 0.6))) * 0.9,
                            3,
                        ),
                        "id": None,
    ...
    def _detect_pattern_route(
        ...
        elif any(k in f"{subject} {object_name} {desc}".lower() for k in ("config", "配置", "setting")) and any(
            k in f"{subject} {object_name} {desc}".lower() for k in ("decision", "决定", "方案")
        ):
    ...
    def generate_candidates(self, limit: int = 20) -> list[dict[str, Any]]:
        ...
        history_keys = set(str(x) for x in history.get("accepted_struct_keys", [])) | set(
            str(x) for x in history.get("rejected_struct_keys", [])
        )
        existing_keys = self._state_keys(state)
        existing_pattern_ids = {
            str(x.get("pattern_id", "")) for x in state.get("candidates", []) if str(x.get("pattern_id", "")).strip()
        }
        ...
        for relation in candidate_relations:
            ...
            base_candidate = {
                ...
                "derivation_rule": "two_hop_rule"
                if int(evidence.get("inference_depth", 1)) >= 2
                else "graph_relation_rule",
                "timestamp": ts,
                "evidence": evidence,
            ...
            if (
                str(ctl.get("admission_decision", "hold")) == "reject"
                or str(ctl.get("knowledge_tier", "observation")) == "rejected"
            ):
                status = "rejected"
            ...
            # Auto triage actions
            triage = self._triage_auto_confirmation(candidate)
            candidate["review_tier"] = str(triage.get("tier", "manual_required"))
            candidate["risk_category"] = str(triage.get("risk_category", "low_risk"))
            candidate["review_note"] = str(triage.get("reason", ""))
            if status == "accepted" and self.auto_accept_enabled and auto_accepted < self.auto_accept_limit:
                if str(triage.get("action", "review")) == "auto_accept":
                    self._accept_candidate_with_meta(
                        candidate,
                        history,
                        auto_approved=True,
                        policy="synthetic_auto_confirm_v1",
                    )
                    auto_accepted += 1
                else:
                    candidate["status"] = "review"
                    candidate["manual_confirmation_required"] = True
            elif status == "rejected":
                self._reject_candidate(candidate, history, reason="auto_rejected_low_score")
            else:
                if not self.reasoning_only_mode:
                    candidate["status"] = "review"
                    candidate["manual_confirmation_required"] = True
        ...
        if any(item.get("status") == "accepted" for item in added):
            try:
                ...
                print(f"[SyntheticMemory] Reindex failed after extraction: {exc}")
    ...
    def record_candidate_hits(
        self, candidate_ids: list[str], used: bool = True, rebuttal: bool = False
    ) -> dict[str, Any]:
        state = self._load_state()
        history = self._load_history()
        pattern_state = self._load_patterns()
        ...
        for item in state.get("candidates", []):
            ...
            if (
                int(item.get("rebuttal_count", 0) or 0) >= self.rebuttal_reject_threshold
                and item.get("status") != "accepted"
            ):
                self._reject_candidate(item, history, reason="rebuttal_threshold")
                rejected += 1
            elif (
                item.get("status") in {"review", "candidate_reasoning_only"}
                and int(item.get("hit_count", 0) or 0) >= self.promotion_min_hits
            ):
                if self.reasoning_only_mode:
                    item["status"] = "promotion_ready"
                    item["promotion_ready_at"] = datetime.now(timezone.utc).isoformat()
                    promoted += 1
                elif float(item.get("scores", {}).get("total", 0.0) or 0.0) >= max(
                    self.review_threshold, self.accept_threshold - 0.03
                ):
                    self._accept_candidate(item, history)
                    promoted += 1
            if self.feedback:
                self.feedback.record_usage(
                    ...
                    extra={
                        "channel": "synthetic_context",
                        "promotion_events": 1 if promoted > 0 else 0,
                    },
                )
            updated += 1
        ...
        if promoted > 0:
            try:
                ...
                print(f"[SyntheticMemory] Reindex failed after evolution: {exc}")
    ...
    def review_candidates(self, decisions: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if accepted > 0:
            try:
                ...
                print(f"[SyntheticMemory] Reindex failed after manual review: {exc}")
    ...
    def confirm_candidates(
        ...
        if accepted_rows:
            try:
                ...
                print(f"[SyntheticMemory] Reindex failed after confirm_candidates: {exc}")
    ...
    def _iter_auto_approved_in_window(self, since_hours: int) -> list[dict[str, Any]]:
        state = self._load_state()
        cutoff = datetime.now(timezone.utc) - timedelta(hours=max(1, int(since_hours)))
        selected: list[dict[str, Any]] = []
        for item in state.get("candidates", []):
            if item.get("status") != "accepted" or not bool(item.get("auto_approved", False)):
                continue
            accepted_at = str(item.get("auto_approved_at", item.get("accepted_at", "")) or "")
            if not accepted_at:
                continue
            raw = accepted_at[:-1] + "+00:00" if accepted_at.endswith("Z") else accepted_at
            try:
    ...
    def preview_rollback_auto_approved(self, since_hours: int = 1) -> dict[str, Any]:
        selected = self._iter_auto_approved_in_window(since_hours)
        return {
            "status": "success",
            "since_hours": int(since_hours),
            "count": len(selected),
            "candidate_ids": [str(x.get("candidate_id", "")) for x in selected][:200],
        }

    def rollback_auto_approved(self, since_hours: int = 1) -> dict[str, Any]:
        state = self._load_state()
        selected_ids = set(self.preview_rollback_auto_approved(since_hours).get("candidate_ids", []))
        if not selected_ids:
            return {"status": "success", "rolled_back": 0, "since_hours": int(since_hours)}
        rolled_back = 0
        touched = []
        for item in state.get("candidates", []):
            cid = str(item.get("candidate_id", ""))
            ...
            item["revoked_reason"] = f"auto_approval_rollback_{int(since_hours)}h"
            item["usage_permission"] = {"recall": False, "inject": "none", "speak": "deny"}
            item["admission_decision"] = "revoke"
        ...
        try:
            ...
            print(f"[SyntheticMemory] Reindex failed after rollback: {exc}")
        self._append_auto_approval_audit(
            {
                "event": "rollback_auto_approved",
                "since_hours": int(since_hours),
                "rolled_back": int(rolled_back),
                "candidate_ids": touched[:200],
            }
        )
        return {
            "status": "success",
    ...
    def health_report(self) -> dict[str, Any]:
        ...
        if kg:
            try:
                ...
                print(f"[SyntheticMemory] Knowledge graph quality stats unavailable: {exc}")
