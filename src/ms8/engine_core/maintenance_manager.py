...
class MaintenanceManager:
    ...
    def _maybe_encrypt_backup_file(self, path: Path) -> None:
        ...
        except (CryptoLockedError, OSError, RuntimeError, TypeError, ValueError):
            return
        if secured != raw:
    ...
    def sync_memory_md_from_auto_memory(self, limit: int = 80) -> dict[str, Any]:
        ...
        if not fresh:
            skip_out: dict[str, Any] = {
                "status": "skipped",
                "reason": "no_new_high_quality_records",
                "candidate_count": 0,
            }
            self._append_sync_audit(skip_out)
            return skip_out
        ...
        for row in fresh:
            ...
            try:
                ...
                try:
                    ...
                    text = (
                        str(redact_sensitive_text(base_text).get("redacted_text", base_text)).replace("\n", " ").strip()
                    )
                except (ImportError, AttributeError, TypeError, ValueError):
                    text = base_text
            text = text[:180]
        ...
        state["synced_auto_memory_ids"] = list(synced_ids)[-1000:]
        self._save_state(state)
        success_out: dict[str, Any] = {"status": "success", "synced": len(fresh), "candidate_count": len(fresh)}
        self._append_sync_audit(success_out)
        return success_out
    ...
    def run_restore_drill(self) -> dict[str, Any]:
        ...
        checks = {
            "has_memory_md_backup": any(
                Path(p).name.lower().startswith("memory") and Path(p).suffix == ".md" for p in restored
            ),
