...
def _maturity_gate(report: dict[str, Any]) -> dict[str, Any]:
    ...
    memory = domains.get("memory", {"pass_rate": 0.0, "fail": 0, "error": 0, "total": 0})
    ...
    security_total = int(security.get("total", 0) or 0)
    connect_total = int(connect.get("total", 0) or 0)
    memory_total = int(memory.get("total", 0) or 0)

    gate: dict[str, Any] = {
        # Memory readiness keeps strict fail/error gate; allow true when no memory checks ran.
        "memory_ready": bool(
            (memory_total == 0) or (memory_critical_fail == 0 and memory_critical_error == 0)
        ),
        # Security/connect should be evaluated only when those domains are present
        # at current check level (L1/L2/L3/L4/FULL_PLUS).
        "security_ready": bool(
            (security_total == 0)
            or (
    ...
    if domains:
        ...
        for name in ("memory", "security", "connect"):
            ...
            if not isinstance(d, dict):
                continue
            lines.append(
                f"- `{name}` total={d.get('total', 0)} "
                f"pass={d.get('pass', 0)} warn={d.get('warn', 0)} "
                f"fail={d.get('fail', 0)} error={d.get('error', 0)} "
                f"pass_rate={d.get('pass_rate', 0)}"
            )
        lines.append("")
    if gate:
...
def _update_alert_state(
    report: dict[str, Any], alert_state_path: Path, cooldown_hours: int = 6, max_per_day: int = 3
) -> dict[str, Any]:
    state: dict[str, Any] = {}
...
def build_health_card(core: Any, snapshot_reason: str = "schedule") -> dict[str, Any]:
    ...
    self_check_settings = (
        mem_settings.get("self_check", {}) if isinstance(mem_settings.get("self_check", {}), dict) else {}
    )
    connect_settings = mem_settings.get("connect", {}) if isinstance(mem_settings.get("connect", {}), dict) else {}
    connect_root = Path(
        str(connect_settings.get("root", os.environ.get("OPENCLAW_MEMORY_AUTO_ROOT", "~/.ms8/connect")))
    ).expanduser()
...
def _diff_health_card(previous: dict[str, Any], current: dict[str, Any]) -> dict[str, Any]:
    ...
    if not prev_files and cur_files:
        ...
        _add(
            "meta.card_version",
            prev_meta.get("card_version", 1),
            cur_meta.get("card_version", 2),
            "migrated",
            "info",
        )
    ...
    for key in sorted(set(prev_services.keys()) | set(cur_services.keys())):
        ...
        if a != b:
            sev = (
                "warning"
                if str(a) in {"running", "True", "true"} and str(b) in {"stopped", "False", "false"}
                else "info"
            )
    ...
    if a_sealed != b_sealed:
        _add(
            "runtime.shadow_sealed",
            a_sealed,
            b_sealed,
            "changed",
            "warning" if b_sealed else "info",
        )
...
def persist_health_card(
    ...
    if sealed and not force:
        return {
            ...
            "history": "",
            "history_count": len(list((memory_dir / "health_card_history").glob("*.json")))
            if (memory_dir / "health_card_history").exists()
            else 0,
            "skipped": True,
