...
class MemoryLearning:
    ...
    def __init__(self, knowledge_graph: KnowledgeGraph | None = None, memory_core=None):
        ...
        fast_start = str(os.environ.get("OPENCLAW_MEMORY_FAST_START", "")).strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
        ...
        if self.learning_enabled and not fast_start:
            self._setup_scheduled_tasks()
            self._start_scheduler_loop()
            self._maybe_bootstrap_meta_task()

    def _meta_log_path(self) -> Path | None:
    ...
    def _maybe_bootstrap_meta_task(self) -> None:
        ...
        if log_path and log_path.exists() and log_path.stat().st_size > 0:
            try:
                ...
                print(f"[Learning] Failed reading bootstrap meta log {log_path}: {exc}")
        try:
            self._run_meta_cognition_task()
        except (AttributeError, OSError, RuntimeError, TypeError, ValueError) as exc:
            print(f"[Learning] Meta-cognition bootstrap task failed: {exc}")
    ...
    def _setup_scheduled_tasks(self):
        ...
        getattr(schedule.every(), compression_day.lower()).do(self._run_weekly_compression_with_log).tag(
            "memory_learning_weekly"
        )
        ...
        if meta_cfg.get("enabled", False) and self.memory_core:
            ...
            if interval_hours and interval_hours >= 1:
                ...
                schedule.every().day.at(meta_cfg.get("schedule_time", "04:00")).do(self._run_meta_cognition_task).tag(
                    "memory_learning_meta"
                )
        ...
        if self.memory_core and bool(self.config["settings"]["memory"]["learning"].get("context_opt_enabled", True)):
            ...
            schedule.every(max(1, interval_hours)).hours.do(self._run_context_optimization_task).tag(
                "memory_learning_context_opt"
            )
    ...
    def _log_learning_event(self, event: str, status: str, detail: dict[str, Any] | None = None) -> None:
        payload = {
            "timestamp": datetime.datetime.now().isoformat(),
            "event": event,
            "status": status,
        ...
        try:
            ...
            print(f"[Learning] Failed writing learning event log {log_path}: {exc}")
    ...
    def _start_scheduler_loop(self) -> None:
        ...
        self._log_learning_event(
            "scheduler", "started", {"poll_seconds": poll_seconds, "instance_id": self._instance_id}
        )
    ...
    def _meta_task_log(self, payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            print(f"[Learning] Failed writing meta task log {log_path}: {exc}")
        ...
        while True:
            ...
            try:
                ...
                if attempt > len(retries):
                    logger.warning("[MemoryLearning] meta cognition task error: %s", exc)
                    return
    ...
    def daily_learning_task(self):
        ...
        try:
            ...
            try:
                from .admission_compat import redact_sensitive_text
        ...
        if self.knowledge_graph and self.knowledge_graph.is_enabled():
            try:
                ...
                logger.warning("[MemoryLearning] knowledge graph daily ingest error: %s", exc)
    ...
    def _section_age_days(self, section: dict[str, Any], today: datetime.date) -> int:
        ...
        if raw:
            try:
                ...
                print(f"[Learning] Invalid section estimated_date '{raw}': {exc}")
        return 0
    ...
    def _apply_compression(self, plan: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            print(f"[Learning] Sensitive text redaction unavailable during compression: {exc}")
        ...
        if self.knowledge_graph and self.knowledge_graph.is_enabled():
            try:
                ...
                logger.warning("[MemoryLearning] knowledge graph weekly maintenance error: %s", exc)
    ...
    def _write_compression_report(self, before: str, after: str, stats: dict[str, Any]) -> Path:
        ...
        if quality["score"] < int(self._compression_settings().get("quality_threshold", 80)):
            logger.warning("[MemoryLearning] compression quality warning: %s", quality["score"])
        return report_path
    ...
    def restore_archived_logs(self, date_prefix: str | None = None, limit: int | None = None) -> list[str]:
    ...
    def weekly_compression_task(self):
        ...
        if compression_cfg.get("notify_on_compress", True):
            logger.info("[MemoryLearning] compression completed: %s", result.get("report_path"))
    ...
    def trigger_daily_learning(self, date_str: str | None = None):
        ...
        analysis = self.analyze_daily_log(date_str)
        if not analysis:
            self._log_learning_event("trigger_daily_learning", "skipped", {"reason": "no_analysis", "date": date_str})
            return
        for entity in analysis.get("entities", []):
            self.sqlite_store.add_entity(entity, "learned")
        for subject, predicate, obj in analysis.get("relations", []):
            self.sqlite_store.add_relation(subject, predicate, obj, strength=0.5)
        self._mark_daily_done(date_str)
        ...
        if self.knowledge_graph and self.knowledge_graph.is_enabled():
            try:
                ...
                logger.warning("[MemoryLearning] knowledge graph batch extract error: %s", exc)
    ...
    def build_memory_tiering_plan(self, retention_days: int | None = None) -> list[dict[str, Any]]:
    ...
    def trigger_memory_tiering(self, retention_days: int | None = None) -> list[str]:
        ...
        manifest: dict[str, Any] = {"moved_files": []}
