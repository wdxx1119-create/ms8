...
class SubAgentManager:
    ...
    def __init__(self, memory_core=None):
        ...
        self.built_in_subagents = [
            ...
            SubAgent(
                ...
                ["memory_edit", "analyze", "summarize"],
            ),
            # 新增 3 个高价值功能
            SubAgent("general-purpose", "Handle any complex task with automatic tool selection", ["all"]),
            SubAgent(
                "history-analyzer",
                "Analyze long-term conversation patterns and user habits",
                ["search", "analyze", "pattern_detect", "statistics"],
            ),
            SubAgent(
                "init",
                "Fast conversation initialization with context loading",
                ["memory_load", "context_setup", "preload"],
            ),
        ]
    ...
    def _find_subagent(self, name: str) -> SubAgent | None:
        """查找子智能体"""
        ...
        try:
            while attempt <= self.max_retries:
                try:
                    ...
                    self._record_task_event(subagent.name, task, status, duration, None)
                    return payload
                ...
                except (RuntimeError, ValueError, TypeError, OSError, asyncio.CancelledError) as e:
                    if attempt > self.max_retries:
    ...
    def _record_task_event(
        self, subagent: str, task: str, status: str, duration: float | None, error: str | None
    ) -> None:
        payload = {
        ...
        try:
            ...
            print(f"[EnhancedSubagents] Failed writing subagent log {path}: {exc}")
    ...
    async def _execute_history_analyzer(self, task: str) -> dict:
        ...
        """
        if not self.memory_core:
            return {"status": "error", "error": "MemoryCore not available for history analysis"}
    ...
    async def _execute_init(self, task: str) -> dict:
        ...
        """
        if not self.memory_core:
            return {"status": "error", "error": "MemoryCore not available for initialization"}
    ...
    def create_custom_subagent(
        ...
        """Create a custom subagent markdown definition compatible with legacy manager API."""
        md_file = self.subagents_dir / f"{name}.md"
        content = (
            f"---\nname: {name}\ndescription: {description}\ntools: "
            f"{', '.join(tools) if tools else 'all'}\n---\n\n{instructions}\n"
        )
