...
class SQLiteMemoryStore:
    ...
    def _mirror_relation_to_kg(
        self, subject_name: str, predicate: str, object_name: str, strength: float = 1.0
    ) -> None:
        if not self._kg_bridge_enabled or not self.kg_db_path:
            return
        now = datetime.now().isoformat()
        try:
            ...
            with sqlite3.connect(self.kg_db_path) as conn:
                ...
                cur.execute(
                    ...
                    (self._canonical_key(subject_name),),
                )
                s = cur.fetchone()
                cur.execute(
                    "SELECT id FROM entities WHERE name_key = ?",
                    (self._canonical_key(object_name),),
                )
                o = cur.fetchone()
                if not s or not o:
                    return
        ...
        if self._kg_bridge_enabled and self.kg_db_path:
            try:
                with sqlite3.connect(self.kg_db_path) as conn:
                    ...
                    aggregated.extend(
                        [(obj_name, predicate, strength) for obj_name, predicate, strength in cursor.fetchall()]
                    )
            except (sqlite3.Error, ValueError) as exc:
                print(f"[SQLiteStore] Failed collecting graph relations for entity '{entity_name}': {exc}")
                        ...
                        with sqlite3.connect(self.kg_db_path) as kg_conn:
                            ...
                            kg_cur.execute(
                                (
                                    ...
                                    "updated_at = ? WHERE name_key = ?"
                                ),
                                (datetime.now().isoformat(), self._canonical_key(entity_name)),
                            )
                            kg_conn.commit()
                    except (sqlite3.Error, ValueError) as exc:
                        print(f"[SQLiteStore] Failed updating KG access count for entity '{entity_name}': {exc}")
