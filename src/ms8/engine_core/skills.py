                    ...
                    if resources_dir.exists():
                        for resource_file in resources_dir.glob("*"):
                            with open(resource_file, encoding="utf-8") as f:
                                self.content["resources"].append({"name": resource_file.name, "content": f.read()})

                    return True
                except (OSError, RuntimeError, TypeError, ValueError, json.JSONDecodeError, yaml.YAMLError) as e:
...
class SkillManager:
                ...
                if skill_dir.is_dir() and (skill_dir / "SKILL.md").exists():
                    ...
                    if skill.load():
                        ...
                        if "frontmatter" in skill.content:
                            desc = self._extract_yaml_value(skill.content["frontmatter"], "description")
                            if desc:
    ...
    def create_skill(
        ...
        tags = self._dedupe_preserve_order([str(item) for item in raw_tags] if isinstance(raw_tags, list) else [])
        triggers = self._dedupe_preserve_order(
            [str(item) for item in raw_triggers] if isinstance(raw_triggers, list) else []
        )
        category = str(metadata.get("category", "workflow"))
        tools = self._dedupe_preserve_order([str(item) for item in raw_tools] if isinstance(raw_tools, list) else [])
...
"""
    ...
    def learn_skill_from_trajectory(
        ...
        return self.create_skill(
            ...
            scope="project",
            resources=extracted.get("resources") if isinstance(extracted.get("resources"), dict) else None,
            metadata=extracted.get("metadata") if isinstance(extracted.get("metadata"), dict) else None,
        )

    def _extract_pattern_from_trajectory(
        self,
        trajectory: list[dict],
        skill_name: str,
        instructions: str | None = None,
    ...
    def _build_structured_skill_body(
        self,
        skill_name: str,
    ...
    def delete_skill(self, name: str) -> dict:
        ...
        try:
            ...
            self.skills = self._load_all_skills()
            return {"status": "success", "message": f'Deleted skill "{name}"'}
        except OSError as e:
            return {"status": "error", "error": f"Failed to delete skill: {str(e)}"}
