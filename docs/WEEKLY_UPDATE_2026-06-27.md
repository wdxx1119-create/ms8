...
本周的 macOS 端工作主要集中在两块增强能力：

- 本地 absorb / project-memory 的增强能力
- 面向日常 Agent 使用的更完整 MCP 记忆面

这次工作的目标是补强 macOS 主线能力，同时保持运行面可测试、可发布、可继续演进。
...
这次对 project-memory 采用整块增强方式接入，使 repository、scanner、parser、health、scope、submit、search、watch 和 CLI 面保持一致。

## MCP 全量记忆面增强

当前 macOS 工作区已经带上了更完整的 MCP 记忆面，而不是旧版偏兼容层的窄接口。
